...
import type { FormatRule, FormatTone } from "./types"
...
const rule = (
  r: Partial<FormatRule> & Pick<FormatRule, "conditions"> & { tone: FormatTone },
): FormatRule => ({
  combinator: "and",
  targets: [],
  targetSections: [],
...
/* ── C-3|伺服器強制的效果(OQ-CF-12 ~ 14)────────────────────────── */

describe("條件式必填", () => {
  const req = (value: unknown): FormatRule => ({
    combinator: "and",
    conditions: [{ field: "狀態", op: "eq", value }],
    targets: ["金額"],
    targetSections: [],
  ...
  it("命中 → 該欄必填", () => {
    const st = evaluateFieldStates([req("送審")], { 狀態: "送審" }, FIELDS)
    expect(st.get("金額")?.required).toBe(true)
    expect(evaluateFieldStates([req("送審")], { 狀態: "草稿" }, FIELDS).size).toBe(0)
  })

  /* 🔴 與唯讀**不同**:官方只對唯讀說「條件式格式必會優先於欄位屬性設定」,
     對必填說的是「已套用欄位必填設定時,設定條件式格式則無法選擇該欄位」——
     即設計期排除,而不是執行期覆蓋。故靜態必填不會被規則拿掉。 */
  it("🔴 必填是聯集不是覆蓋 —— 規則沒講到時,靜態必填仍然成立", () => {
    expect(resolveFieldAttrs({ required: true }, undefined).required).toBe(true)
    expect(resolveFieldAttrs({ required: true }, { tone: "warn" }).required).toBe(true)
    expect(resolveFieldAttrs({ required: false }, { required: true }).required).toBe(true)
  })

  /* 官方逐字:「當欄位因條件式格式被隱藏時,系統會略過檢查必填及輸入檢查」 */
  it("🔴 被規則隱藏 → 連靜態必填一起放掉(否則使用者卡死在看不見的欄位上)", () => {
    const attrs = resolveFieldAttrs({ required: true }, { hidden: true, required: true })
    expect(attrs.required).toBe(false)
    expect(attrs.skipValidation).toBe(true)
  })

  it("靜態隱藏不放掉必填 —— 兩者成因不同", () => {
    expect(resolveFieldAttrs({ required: true, hidden: true }, undefined).required).toBe(true)
  })
})

describe("動作按鈕與開始簽核的閘門", () => {
  const gateRule = (over: Partial<FormatRule> & Pick<FormatRule, "effects">): FormatRule => ({
    combinator: "and",
  ...
  it("按鈕:命中才上鎖,理由取同一條規則上的訊息", () => {
    const r = gateRule({
      targetButtons: [7],
      effects: [
        { kind: "readonly" },
        { kind: "message", text: "金額 {{fieldValue:金額}} 需先簽核" },
      ],
    })
    const hit = evaluateButtonGate([r], { 金額: 20000 }, FIELDS, 7)
    expect(hit.locked).toBe(true)
    expect(hit.message).toBe("金額 20000 需先簽核")

    expect(evaluateButtonGate([r], { 金額: 5000 }, FIELDS, 7).locked).toBe(false)
    /* 只套用到指定的按鈕 —— 別的按鈕不受影響 */
    expect(evaluateButtonGate([r], { 金額: 20000 }, FIELDS, 8).locked).toBe(false)
  })

  it("開始簽核:targetApproval 才算數", () => {
    const r = gateRule({ targetApproval: true, effects: [{ kind: "hide" }] })
    expect(evaluateApprovalGate([r], { 金額: 20000 }, FIELDS).hidden).toBe(true)
    expect(evaluateApprovalGate([r], { 金額: 900 }, FIELDS).hidden).toBe(false)
    /* 同一條規則沒有勾「開始簽核」時不得外溢到簽核 */
    const b = gateRule({ targetButtons: [7], effects: [{ kind: "hide" }] })
    expect(evaluateApprovalGate([b], { 金額: 20000 }, FIELDS).hidden).toBe(false)
  })

  it("沒有規則 → 全開(不得預設擋住)", () => {
    expect(evaluateButtonGate([], {}, FIELDS, 7)).toEqual({
      hidden: false,
      locked: false,
      message: null,
    })
