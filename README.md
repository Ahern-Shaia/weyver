...
| **04** | **[完整產品功能表](docs/04-完整產品功能表.md)** | ✅ **2026-07-19 v2.5**(**一體 offering 缺一不可**;**480 MVP OSS-only**|v2.5 通用功能審計 +8:租戶設定中心 / 使用者管理 + 外部使用者 / 個人設定 / 回收桶 / 帳號安全 / PDPA;前 v2.4 docs/25 審計補遺 +32;Weyver 自建全套 ERP CORE)|
| — | [策略簡報](docs/strategy-slides.html) | ✅ 2026-07-18 重刷(對齊 v2.3 / solo 時程 / 全自研 TS / OSS / 13 張)|
| **07** | **[產品開發時程規劃書](docs/07-產品開發時程規劃.md)** | ✅ **2026-07-19 v2.12**(**§4 逐月排程重編為 Ragic-first**|表單引擎核心→關聯/權限→pilot Ragic 遷移,移除 Odoo/AG Grid/Q/GL/電子發票(排 Phase 2);§0 表 + LOC + 成本對齊 docs/04 v2.5 480(時程結論不變);OSS-only 砍商業授權;§ 1.4 Vernova 實測校準保留)|
...
| **⭐ 13** | **[產品開發功能順序表](docs/13-產品開發功能順序表.md)** | ✅ **2026-07-19 v1.6**(**buildable sprint 順序**|Phase 1 Ragic ~236 → Phase 2 ERP +~204 → Phase 3 MES/ISO +40 = 480;v1.6 v2.5 補遺 cascade(P1-I ~87);v1.5 §3.1 P0-1 後端核心 SHIPPED;dependency + Ragic-first M18 + risk gates)|
...
| **⭐ 23** | **[產品功能發布藍圖(客戶版)](docs/23-產品功能發布藍圖.md)** | ✅ **2026-07-19 v6.3**(v2.5 補遺 cascade;**§1.6 開發啟動狀態:Phase 1 工程起跑,R1 命門已交付並通過驗證,時程承諾不變**;Ragic-parity-first:R1 完整 Ragic(~236,含 AI 建表/NL/ZEN + 設定中心)→ R2 ERP+計算層(+~204)→ R3 MES+ISO(+~40,完整 MVP 480)→ R4 對標+AI;solo 務實 R1 ~2-2.5 年 / R3 ~4-4.5 年)|
| **⭐⭐ 24** | **[用戶心智模型與設計原點](docs/24-用戶心智模型與設計原點.md)** | ✅ **2026-07-19 v1.3**(視覺之上游依據|Weyver 用戶=Ragic 範式思考者,**主要畫面=「自己建自己填」的表單資料庫非 KPI 儀表板**;客戶校準:ERP/MES/ISO **織入同一 Ragic 工作區、非每系統一 tab**;§6 前端 surface 地圖 **S1–S22**(v1.3 補 S22 設定中心))|
| **⭐ 25** | **[功能完整對照清單](docs/25-功能完整對照清單.md)** | ✅ **2026-07-19 v1.2**(開發對照權威 checklist|A–U 全子功能 × R1–R4 × S1–S22;v1.2 v2.5 通用功能補遺(480);v1.1 8 缺口 + 5 衝突已全數拍板併入 docs/04 v2.4 並 cascade 13/23/README)|
