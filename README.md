...
| **04** | **[完整產品功能表](docs/04-完整產品功能表.md)** | ✅ **2026-08-08 v2.8**(**一體 offering 缺一不可**;**488 MVP OSS-only**|v2.8 Q 收斂落地 20→5(docs/35 OQ-R2M-1)|v2.7 ERP-parity 缺口 +19:委外/票據/信用/營業稅/SoD/連接器(全 R2);v2.6 設定頁 +4;v2.5 通用功能 +8;Weyver 自建全套 ERP CORE)|
| — | [策略簡報](docs/strategy-slides.html) | ✅ 2026-07-18 重刷(對齊 v2.3 / solo 時程 / 全自研 TS / OSS / 13 張)|
| **07** | **[產品開發時程規劃書](docs/07-產品開發時程規劃.md)** | ✅ **2026-08-08 v2.15**(v2.8 對齊 488 / Phase 2 +~208,時程結論不變;**§4 逐月排程重編為 Ragic-first**|表單引擎核心→關聯/權限→pilot Ragic 遷移,移除 Odoo/AG Grid/Q/GL/電子發票(排 Phase 2);§0 表 + LOC + 成本對齊 docs/04 v2.7 503(時程結論不變);OSS-only 砍商業授權;§ 1.4 Vernova 實測校準保留)|
...
| **⭐ 13** | **[產品開發功能順序表](docs/13-產品開發功能順序表.md)** | ✅ **2026-08-08 v1.9**(**buildable sprint 順序**|v1.9 Q 收斂:P1-A 改匯入 uplift ~5、首棒讓位 P1-B J 財會;Phase 1 Ragic ~240 → Phase 2 ERP +~208 → Phase 3 MES/ISO +40 = 503;v1.8 v2.7 ERP-parity cascade;v1.5 §3.1 P0-1 後端核心 SHIPPED;dependency + Ragic-first M18 + risk gates)|
...
| **⭐ 23** | **[產品功能發布藍圖(客戶版)](docs/23-產品功能發布藍圖.md)** | ✅ **2026-08-08 v6.6**(Q 收斂落地 R2 +~208 / 總 488;v2.7 ERP-parity cascade;**§1.6 開發啟動狀態:Phase 1 工程起跑,R1 命門已交付並通過驗證,時程承諾不變**;Ragic-parity-first:R1 完整 Ragic(~240,含 AI 建表/NL/ZEN + 完整設定中心)→ R2 ERP+計算層+ERP-parity(+~223,委外/票據/信用/營業稅/SoD/連接器)→ R3 MES+ISO(+~40,完整 MVP 503)→ R4 對標+AI;solo 務實 R1 ~2-2.5 年 / R3 ~4-4.5 年)|
...
| **⭐ 25** | **[功能完整對照清單](docs/25-功能完整對照清單.md)** | ✅ **2026-08-08 v3.5**(開發對照權威 checklist|A–U 全子功能 × R1–R4 × S1–S22 + A–I 覆蓋率逐段校對;v3.5 Q 段收斂 20→5;v1.1–1.3 缺口已全數拍板併入 docs/04 並 cascade 13/23/README)|
...
| **28** | **[Metabase 設計分析](docs/28-Metabase-設計分析.md)** | ✅ **2026-08-01 v1.0**(視覺 × 互動;數值與規則**取自 Metabase 原始碼直讀**非二手,逐項給 Weyver 裁定。高價值三項:色階由品牌色 `color-mix` 推導(解白牌維護量)/ 色碼字面值 lint(**我方現況是破口**)/ 步驟式建構器(R2「算的自助化」命門)。**明確不搬**:圓角 8px、靜態陰影、儀表板式 IA)|
| **⭐⭐ 34** | **[開源 MES 生態技術與功能拆解](docs/34-開源MES生態技術與功能拆解.md)** | ✅ **2026-08-03 v1**(T 模組之「站在巨人的肩膀上」;六軸平行研究,承重授權**逐一複驗 LICENSE 檔本文**。① 功能最完整的 qcadoo 是 AGPL 不可讀、可 fork 的 UMH 根本不是 MES ② 開源 MES 圈**無終端使用者自助設計器**(向上位置可稽核)③ 🔴 **EMQX 轉 BSL 推翻 docs/11 選型** → NanoMQ(MIT)④ 功能矩陣軸改 ISA-95 P3 4×8 ⑤ **B2MML = 免費取得 ISA-95 物件模型** ⑥ APS 無 JS/TS CP-SAT,待 M0 裁定。另記 4 起授權漂移 + 3 條不得宣稱事項)|
| **⭐ 35** | **[R2 ERP 計算層架構總圖](docs/35-R2-ERP計算層架構總圖.md)** | ✅ **2026-08-08 v1.0 APPROVED**(R2 design-ahead 總圖|五層架構 + 依賴圖(J.gl 匯點)+ 十項共用原語 + R1 預留地基 P1–P10 + M0 進場順序 J.gl 最先;OQ-R2M-1 Q 收斂 20→5)|
| **38** | **[前端設計規則](docs/38-前端設計規則.md)** | ⚠️ **2026-08-08 降級為「再驗證與延伸」** —— 規則本體以 `~/Documents/design-system/DESIGN-RULES.md` 為準(同批截圖但更早、更完整)。本檔獨有仍有效:§6 色彩限制解除 · §5.6–5.9(空狀態 / 設計器 / 型別圖示 / 側欄)|
| **⭐ 39** | **[視覺落差與對齊計畫](docs/39-視覺落差與對齊計畫.md)** | ✅ **2026-08-08 v1**(把「為什麼那份 mockup 像 SaaS 不像後台」拆成可執行落差:**六機制**(近黑主按鈕 / 單一強調色 / 階層靠字重 1.43× / 格線比外框淺 / 資料擁有顏色 / 狀態機優先於 CRUD)+ 11 項落差 + **6 步計畫**(字體統一刻意排最後)。🔴 兩個未解:上游權威 `DESIGN-RULES.md` **在 repo 外不在 CI**;E 案型別分組色與「資料擁有顏色」相衝待裁定)|
