...
export function BarcodeView({
  ...
  if (text === "") return <span className="text-[11px] text-ink-4">—</span>
  if (symbology === "code128") {
    return (
      <span className="inline-flex items-center gap-1 rounded-xs border border-line bg-label px-1.5 py-0.5 text-[10px] text-ink-4">
  ...
  return (
    <span className="inline-flex flex-col items-start gap-0.5">
      ...
      <span className="font-mono text-[9.5px] text-ink-4">{text}</span>
    </span>
  )
}
