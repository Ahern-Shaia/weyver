...
function FormCard({ form }: { readonly form: FormSummary }): ReactNode {
  if (form.locked) {
    return (
      <div
        title="無存取權,請洽管理員授予"
        className="cursor-not-allowed rounded-md border border-dashed border-line bg-surface px-3.5 py-3 opacity-70"
      >
    ...
    <div className="h-full overflow-y-auto">
      <div className="mx-auto max-w-5xl px-8 py-7">
        <div className="flex items-baseline gap-3">
          ...
          <Link
            href="/app/builder"
            className="ml-auto flex h-9 items-center gap-1.5 rounded-md bg-primary px-3.5 text-[14px] font-medium text-white transition-colors duration-fast-01 ease-productive-exit hover:bg-primary-d"
          >
        ...
        <p className="mt-5 border-l-2 border-primary py-0.5 pl-3.5 text-[14px] text-ink-3">
        ...
        {isPending ? (
          <div className="mt-8 text-[14px] text-ink-3">載入表單…</div>
        ) : isError ? (
