      ...
      <table className="w-full border-collapse text-[12px]">
        <thead>
          <tr className="bg-head">
            <th className="w-8 px-2 py-2 text-left font-mono text-[10.5px] font-semibold text-ink-4">
              #
            </th>
            {cols.map((f) => (
              <th
        ...
        <tbody>
          {lines.map((r, i) => (
            <tr key={r.id} className="border-t border-line-2">
              <td className="px-2 py-1.5 font-mono text-[10.5px] text-ink-4">{i + 1}</td>
              {cols.map((f) => (
                <td
                  key={f.id}
                  className={
          ...
          {lines.length === 0 ? (
            <tr>
              <td colSpan={cols.length + 1} className="px-3 py-3 text-[11.5px] text-ink-4">
                此單據尚無明細。
              </td>
            </tr>
          ) : null}
