...
test("看板:純鍵盤即可換欄,且一次按鍵跳一整欄", async ({ page, request }) => {
  ...
  /* 🔴 **位移是非同步生效的,放下之前必須先等它生效**。
     ...
     📊 量測:修前單跑 4 次過 2 次;修後單跑 **20 次全過**,整套連兩次全綠。
     ⚠️ 修後仍在整套跑觀察到**一次**失敗(ArrowRight 5 秒內無效)。
     同一次的診斷顯示欄位矩形完全正常(甲@208 乙@456 丙@704,卡片@228),
     故**不是版面問題**;真因**未確認**,若再現請先 dump live region 逐步驟狀態。 */
