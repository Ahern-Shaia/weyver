...
| 項目 | 數字 | 來源 |
|---|---|---|
| **完整 MVP 總人月(路線 A + 全自研 TS + OSS-only + 通訊平台 + 審計補遺)** | **480** | docs/04 **v2.5**(2026-07-19 通用功能審計:+設定中心/使用者管理/外部使用者/個人設定/回收桶/帳號安全/PDPA 8;前 v2.4 docs/25 審計 +32)|
| ├── Ragic 側(A–I 9 模組,含 Glide Data Grid + 通訊平台通道 + v2.4 AI 建表/公式/NL + ZEN + 白牌/配額 + 可靠性 + v2.5 設定/管理類) | **237** | v2.5 +8 vs v2.4 |
...
| **Solo|R1 完整 Ragic 平台**(~236 人月 v2.5,Ragic-parity-first,既有客戶遷移 land)| **務實 ~2-2.5 年 / 加速 ~1.5 年** | docs/23 v6.3 + docs/07 §1.4(8 人團隊對照 ~2.2 年)|
...
| **04** | **[完整產品功能表](docs/04-完整產品功能表.md)** | ✅ **2026-07-19 v2.5**(**一體 offering 缺一不可**;**480 MVP**(v2.5 通用功能審計 +8:租戶設定中心 / 使用者管理 + 外部使用者 / 個人設定 / 回收桶 / 帳號安全 / PDPA;前 v2.4 docs/25 審計補遺 +32);OSS-only;solo 時程 band 不變 務實 ~4-4.5 年 / 加速 ~3 年)|
...
| **07** | **[產品開發時程規劃書](docs/07-產品開發時程規劃.md)** | ✅ **2026-07-19 v2.12**(v2.5 補遺對齊|480 / Phase 1 ~236,**時程結論不變**;v2.11 §4 逐月排程由舊 Odoo-fork/Q-first 完整重編為 Ragic-first|月段 A-E 改表單引擎核心→關聯/權限→pilot Ragic 遷移,移除 Odoo/AG Grid/Q read/GL/電子發票 stub(排 Phase 2);§5 成本砍商業授權+Odoo 顧問(~800K-1.45M→~150K-400K);前 v2.9 calendar 稀釋校正|主導因子為穩定期 ~30% 維運拖累非 coding 速度)|
...
| **⭐ 13** | **[產品開發功能順序表](docs/13-產品開發功能順序表.md)** | ✅ **2026-07-19 v1.6**(480 MVP 拆 sprint;v1.6 v2.5 補遺 cascade|Phase 1 ~236 / P1-I ~87;dependency 圖;v1.5 §3.1 P0-1 後端核心 SHIPPED;v1.4 §3/§4 Ragic-parity release 重編)|
...
| **⭐⭐ 24** | **[用戶心智模型與設計原點](docs/24-用戶心智模型與設計原點.md)** | ✅ **2026-07-19 v1.3**(**視覺之上游依據**|Weyver 用戶=Ragic 範式思考者,**主要畫面=「自己建自己填」的表單資料庫非 KPI 儀表板**;客戶校準:ERP/MES/ISO 織入同一 Ragic 工作區、**非每系統一個 tab**;§6 前端 surface 地圖 **S1–S22**(v1.3 補 S22 設定中心,Phase 1 = S1–S13+S22 / J–U 織入 Phase 2+);反面教材=為 SaaS 而 SaaS + 分立模組 tab)|
| **⭐ 25** | **[功能完整對照清單](docs/25-功能完整對照清單.md)** | ✅ **2026-07-19 v1.2**(**開發對照權威 checklist**|A–U 全子功能 × R1–R4 × S1–S22;v1.2 v2.5 通用功能補遺(A 27 / E 14 / H 32 / I 24 = 480);v1.1 審計 8 缺口 + 5 衝突已全數拍板併入 docs/04 v2.4;§3 補遺人月定案:AI MVP 16 + ZEN 3 + 明文化 2 + 白牌/配額 3 + 可靠性 8)|
