...
| 項目 | 數字 | 來源 |
|---|---|---|
| **完整 MVP 總人月(路線 A + 全自研 TS + OSS-only + 通訊平台 + 審計補遺)** | **488** | docs/04 **v2.8**(2026-08-08 Q 收斂落地 −15,docs/35 OQ-R2M-1;v2.7 ERP-parity 缺口 +19;v2.6 設定頁 +4;v2.5 通用功能 +8;v2.4 審計 +32)|
| ├── Ragic 側(A–I 9 模組;含設定/管理類 + v2.7 E SoD 3 + G 連接器 4 均 R2-release) | **248** | v2.7 +7 vs v2.6 |
| ├── ERP 側(J–Q 8 模組,全 Weyver 自建)| 185 | J 53 + K 33 + L 36 + M 13 + N 15 + O 13 + P 17 + **Q 5(v2.8 收斂為 onboarding 匯入 uplift)**(v2.7 +委外/票據/信用/營業稅)|
...
| **04** | **[完整產品功能表](docs/04-完整產品功能表.md)** | ✅ **2026-08-08 v2.8**(**一體 offering 缺一不可**;**488 MVP**(v2.8 Q 收斂落地 −15:Q 20→5 onboarding 匯入 uplift,docs/35 OQ-R2M-1;v2.7 ERP-parity 缺口 +19;v2.6 設定頁 +4;v2.5 通用功能 +8;v2.4 審計 +32);OSS-only;solo 時程 band 不變 務實 ~4-4.5 年 / 加速 ~3 年)|
...
| **07** | **[產品開發時程規劃書](docs/07-產品開發時程規劃.md)** | ✅ **2026-08-08 v2.15**(v2.8 Q 收斂對齊|488 / Phase 2 +~208 / Phase 1 ~240,**時程結論不變**;v2.11 §4 逐月排程由舊 Odoo-fork/Q-first 完整重編為 Ragic-first|月段 A-E 改表單引擎核心→關聯/權限→pilot Ragic 遷移,移除 Odoo/AG Grid/Q read/GL/電子發票 stub(排 Phase 2);§5 成本砍商業授權+Odoo 顧問(~800K-1.45M→~150K-400K);前 v2.9 calendar 稀釋校正|主導因子為穩定期 ~30% 維運拖累非 coding 速度)|
| **08** | **[MES 市場分析報告](docs/08-MES-市場分析.md)** | ✅ **2026-07-16 v1**(11 家 MES vendor 分析 + 功能對照矩陣 + Weyver T 模組策略)|
| **09** | **[ERP 市場分析報告](docs/09-ERP-市場分析.md)** | ✅ **2026-08-08 v2**(12 家;台灣三家深掘|**新增千奧 §3.12**(pilot 三套 ERP 之一,萬元級單機買斷)+ **鼎新/正航 AI 查證均有**(AiGP/Agent Space/ChatFile;RAG 客服+Semantic Kernel NL 查詢/AI 產表單)+ ERPNext 授權更正 MIT→GPL-3.0 + §6 對齊「Q 收斂 onboarding 一次匯入」定位)|
...
| **⭐ 13** | **[產品開發功能順序表](docs/13-產品開發功能順序表.md)** | ✅ **2026-08-08 v1.9**(488 MVP 拆 sprint;v1.9 Q 收斂|P1-A 改「Q 匯入 uplift ~5」,**Phase 2 首棒讓位 P1-B J 財會**,Phase 2 +~208;v1.8 v2.7 ERP-parity cascade;v1.5 §3.1 P0-1 後端核心 SHIPPED)|
| **⭐ 14** | **[前端設計規則與技術棧](docs/14-前端設計規則與技術棧.md)** | ✅ **2026-08-08 v7.0**(🔴 **§0.6 前端視覺以 `docs/mockups/weyver-v3.html` 為準** —— 決策方品牌重設計全套採用;**所有數字為實測**:圓角全 0、陰影只有 focus ring 與 overlay、字級 9 階、**字體兩軌**(介面 Noto Sans TC / 識別碼 IBM Plex Mono);🔴 **「display 階」是虛的**(Archivo 只服務「Weyver」四字)、**Inter 在 v3 不存在**。三項交裁定:prod tokens.css 圓角衝突 / 字級半階 / Inter 去留)· 舊述 **v6.0**(§0.4 對照 Metabase 之 token 架構裁定;禁 hex 升級 CI)· 舊述 v2.1(**嚴謹企業級定案,全文改寫**|上游=docs/24 form-first;全框線欄位表 / 方角 2-3px / **禁陰影禁 pill** / 扁平主色分段條 / 帶框狀態章 / 12.5px 密度 / 信任訊號鐵則(時間戳/版本/稽核/借貸平衡/記錄導航/狀態列)/ 動效近乎無;§1.2 套件×surface S1–S21 對映;權威稿=`workspace-form-first-v2.html`,v1 mockups SUPERSEDED;**v2.1 品牌色定案=一套系統三配色主題可切換**(深藍預設/深海青/石墨,`[data-theme]` 語意 token))|
...
| **⭐ 25** | **[功能完整對照清單](docs/25-功能完整對照清單.md)** | ✅ **2026-08-08 v3.5**(**開發對照權威 checklist**|A–U 全子功能 × R1–R4 × S1–S22 + A–I 覆蓋率逐段校對(v3.x 系列以「把帳算對」為主軸);v3.5 Q 段收斂 20→5;⚠️ 本索引先前停在 v1.4 已過期多輪,權威以檔內版本段為準)|
...
| **17** | **[AI-native 設計](docs/17-AI-native-向上設計.md)** | ⚠️ **2026-08-02 v2 重大更正**(**AI 不是向上軸,是 table stakes**)。v1 的核心前提「Ragic/傳統 ERP=0 AI」**經一手查證為錯**——Ragic 實有 AI 建資料庫(需求→自動生成一系列表單+關係圖)/ NL 查詢 / 公式助手 / 單據抽取(拍發票自動填單)/ **AI Agent(6 觸發 × **10** 動作〔2026-08-06 開檔覆查更正:原記 11,把 HTML 表頭列算進去了〕,含 `CREATE_RECORD`/`MODIFY_RECORD`)** / MCP(唯讀繼承權限)/ 私有主機本地模型+BYO key;我方列為「槓桿最高」的四項它全部已有。Ragic 明文空缺僅「無法產出報表」一項。**向上軸移交 docs/18 計算層**(Ragic 沒有「算」→ 不可能有「AI 綁算」;AI 是自助綁定的手段不是目的)。docs/17 功能清單與人月不變(降為 parity),ROI 論述需重估。~~⚠️ 鼎新/正航/千奧是否有 AI 未查證~~ → **2026-08-08 docs/09 v2 查證:鼎新(AiGP/Agent Space/ChatFile)與正航(RAG 客服 + Semantic Kernel NL 查詢/AI 產表單)均有 AI,千奧仍未查證**。教訓入 AGENTS.md〈向上設計三條〉 |
...
| **⭐ 35** | **[R2 ERP 計算層架構總圖](docs/35-R2-ERP計算層架構總圖.md)** | ✅ **2026-08-08 v1.0 APPROVED(OQ-R2M-1..6 全採)**(**R2 design-ahead 總圖**|五層架構(表單 app → calc-binding → 計算引擎 docs/18 + 領域引擎 docs/20 → R1 substrate)+ 模組地圖 J–R + 依賴圖(**J.gl 為全圖匯點,各模組不得自帶過帳**)+ 十項共用原語 + **R1 預留地基 P1–P10(P1–P4 入 R1 backlog)** + M0 進場順序 J.gl→K→L+N→O/M→P/R/G;**OQ-R2M-1 解掉 docs/13 P1-A 與 Q 收斂的矛盾**(Q 20→5);深度停在架構裁定,各 M0 動工強制重走查 §0 快照)|
...
- 2026-08-08|**gl.md v1.1 深度補齊(決策方追問「有做深度研究嗎」→ 誠實盤點差三塊,全補)**|(a) **承重句逐條親驗**([[feedback-verify-load-bearing-claims]] 紀律:不採信 subagent 轉述)—— OFBiz 原檔下載 grep(±0.01 容差 L196/201 · SECA L176-181 · 雙金額 L1886-1890 · 沖轉 D↔C L306-310 · PERIOD_CLOSING L686/700)+ ERPNext 三頁原文 + Odoo 六句行號 + 法條 §17/§38 原文,**三個研究 agent 的轉述零反轉**。(b) **台灣法規一手深查**(law.moj.gov.tw:商業會計法 / 商業使用電子方式處理會計資料辦法 / 商業會計處理準則)—— **兩條改設計**:§17 傳票法定三類(收入/支出/轉帳)→ schema 加 `statutory_type`;§38 帳簿報表**十年保存** → 不變量 8(gl_* 不適用一般 purge,與 PDPA 邊界=法定保存例外)。**三條升級 OQ 依據**:§18 但書「結算後轉入帳目得不檢附原始憑證」= 年結結轉傳票於法有據(OQ-GL-5 原「會計師期待」推定退場)· 電子方式辦法無 hash 鏈明文 = hash 鏈為加分項非義務,且 §6「更正經審核並作成紀錄」正面錨定沖轉+audit(OQ-GL-6)· 準則 §3「商業得因實際需要增訂」= **客戶自訂科目的法源,與表單範式自助精神同向**(OQ-GL-7)。(c) **§7-bis GL 設計難題清單**一手錨定:TigerBeetle hot-account row lock(我方「餘額=SUM 非 balance row」**結構性避開**)· PG deferred constraint trigger(可行,per-row 排隊成本待實測)· append-only 聚合(index-only scan visibility map / BRIN;SUM 無官方量化 → M1 照 dynamic-permissions 30 萬列範式實測)。M0 級未查證清零;評級誠實停 ⭐(零反轉故無「推翻過」,不硬湊 ⭐⭐)。
- 2026-08-08|**R2/gl.md 總帳 GL design-ahead M0 APPROVED(OQ-GL-1..8 全數裁定)**|J.gl 依 docs/35 進場順序開檔。三站研究:**OFBiz 原始碼**(LICENSE 檔本文複驗 Apache-2.0;雙金額/isPosted 生命週期/沖轉 D↔C/期結傳票;**兩個要修的包袱:不可變靠 service 層守〔繞過直寫 DB 無防護〕→ 我方下沉 DB trigger;試算容差 ±0.01 寫死 → 我方容差 0**)+ **ERPNext 只讀官方文件**(Immutable Ledger:cancel=反向分錄不刪、期間硬鎖、PCV)+ **Odoo 本地文檔庫**(三層鎖/年結不產傳票之動態本年度損益/SHA-256 hash 鏈/l10n_tw 台灣 CoA)。落定:五表 schema、七不變量(DB 層強制非 service 禮貌)、**傳票=表單 + 過帳鏡射 Tier-1 ledger**(OQ-GL-1:「一切皆表單」與 DB 級不可變兩全,= ERPNext 單據→GL Entry 兩層)、年結混合案(平時動態損益 + 年結結轉傳票草稿人核准)、hash 鏈預留欄位不實作。**裁定分流首次照 AGENTS ✅ 規則執行**:六條研究錨定自動核准,兩條(年結台灣慣例/hash 鏈法規)因懸未查證斷言交裁均採建議,**未查證處明標**(M1 前問會計師/併 O 模組)。MODULES.md R2 段同步。
- 2026-08-08|**docs/35 R2 ERP 計算層架構總圖 v1.0 APPROVED(design-ahead)+ Q 收斂落地 cascade(503 → 488)**|R1 未完但 ERP 各模組先開 design doc —— 裁定「先出總圖再進單模組」。docs/35:五層架構 / 模組地圖 / 依賴圖(**J.gl 為匯點,一切過帳走同一條 calc-binding → GL pipeline,各模組不得自帶過帳**)/ 十項共用原語 / **R1 預留地基 P1–P10** / M0 進場順序(J.gl 最先)。OQ-R2M-1..6 全採:① **Q 收斂落地** —— 2026-07-17 已定案「Q = onboarding 一次匯入」但 docs/13 P1-A 仍排「Q 對帳 bridge 20 人月」為 Phase 2 首棒(stale 矛盾),裁定 Q 20→5(鼎新/千奧/正航 匯入 adapter + 對映精靈 + 一次性對帳驗證;SAP B1 降對標),**MVP 503 → 488、對標 ~944 → ~908、R2 +223 → +208,Phase 2 首棒讓位 P1-B J 財會,solo band 不變**;④ R1 預留 P1–P4(狀態轉換事件語意 / DDL veto hook / **表單級不可變** / currency numeric 對碼)入 R1 backlog(記 MODULES.md R2 段);② 模組切分 domain module + calc-core;③ design-ahead 深度停在架構裁定,各 M0 動工強制重走查 §0 前提快照;⑤ SoD 併 authz uplift;⑥ R HR 不納本輪。cascade|04 v2.8 · 13 v1.9 · 23 v6.6 · 25 v3.5(順帶更正 CLAUDE 索引停在 v1.4 的過期)· 07 v2.15 · MODULES.md · CLAUDE/README。下一份:J.gl 總帳 design-ahead M0。
- 2026-08-08|**docs/09 v2 台灣 ERP 三家深掘(三路平行研究)+ AI 查證 + 授權更正**|補強商業 vendor 分析。(a) **新增千奧資訊 §3.12** —— pilot 客戶三套 ERP 之一而 v1 完全缺席;查證為萬元級 Windows 單機/區網買斷軟體(kingcard.com.tw 金卡/ERP6,資本額 1,000 萬、32 人;有 MRP/IFRSs 會計/POS;注意與「千奧有限公司」撞名非同一家);遷移阻力最低但競爭基準是「便宜、買斷、夠用」。(b) **清掉 docs/17 的「鼎新/正航 AI 未查證」欠帳:兩家均有 AI** —— 鼎新 AiGP 全產品線(生單/文件總結/報表分析三助手)+ Agent Space + METIS ChatFile;正航 RAG 智能客服 + Semantic Kernel 系統 AI 助理(**NL 查詢 / 自動產生 ERP 表單**,與 Weyver AI 功能直接重疊)—— 三度驗證「競品沒有 X」句型之風險;千奧仍未查證(官網 Big5 未能全站確認,依鐵則不寫「沒有」)。(c) 鼎新產品線更正:五線並存 T100 未取代 GP,補 Cosmos/SmartERP/A1 月租雲(600 元/月起);正航梯隊更名 一號→T357→T8/T9→NBS(v1 記法已過時);兩家食品業方案皆為官方成型(鼎新 HACCP/清真、正航批號追溯+衛福部非追不可對接)。(d) **ERPNext 授權更正 v1 誤記 MIT → GPL-3.0**(§5-bis 複驗)只讀公開文件。(e) §6 修 stale 定位:v1「不推翻現有 ERP、上層 Q 對帳」對齊 2026-07-17「客戶放棄原 ERP、Q 收斂 onboarding 一次匯入」。全部承重結論附官方逐字 + 出處 + 查證日 2026-08-08。cascade|09 v2 · 17(§0 查證更新)· CLAUDE/README。
