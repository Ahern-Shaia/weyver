...
### 🔴 出貨的功能都要有**可回查**的研究(2026-08-06 起由 CI 把關)

`_template.md` 早就要求「研究即寫入 doc」「附**可點擊的來源連結**」,
但那是規則不是檢查 —— 而本 repo 已為「規則寫了沒檢查就會漏」付過六次代價。

`packages/docs-check/src/research-required.test.ts` 現在盯著它,**CI 會跑**。

| | |
|---|---|
| **判準** | 標準證據段(`## 0.` / `## 0-bis.` / `## 0-ter.`,或〈站在巨人的肩膀〉)**且**有出處連結(≥10)或逐字引用(≥5)|
| **形式** | **棘輪**:不合格的已出貨模組數只能往下(基準 14)。補一份就把 `BASELINE` 減一 |
| **為什麼不是硬門檻** | 46 份裡 14 份不合格,一次要求補齊 = CI 第一天就紅,而紅了沒人修的 CI 等於沒有 CI |

🔴 **它檢查的是格式,不是「有沒有做研究」。**
「有沒有研究」機器偵測不出來 —— 同一天誤判四次,其中 `foundation/auth` 被判成
「沒做研究」而它實際上讀了 better-auth 原始碼並**修掉三個 P0 資安漏洞**。
研究可以寫在任何標題底下,繼續加偵測規則只會繼續產生假陰性。

**所以不合格不代表沒做研究,代表別人查不到** —— 而查不到的研究,
它的價值有一半(日後能重新查證)已經沒了。

### 🔴 代理指標的輸出不是答案(2026-08-06,同一天犯三次)

寫一個規則 / 腳本 / 啟發式去「量」某件事,然後**把它的輸出當結論報出去** ——
那一天連續三次都是這個形狀,三次都是**去看實際的東西**才發現錯:

| # | 代理指標 | 錯法 |
|---|---|---|
| 1 | 「程式碼引用競品但 doc 沒查」的關聯檢查(用目錄名比對) | **假陽性與假陰性都有**:唯一報出來的是假陽性,而真缺口沒抓到 |
| 2 | FMEA「改欄位名 → 整張表存不了」 | 推論鏈每一環都對,**而系統根本沒有改名端點** |
| 3 | 研究強度(白名單競品名 + 特定標題) | 把三份研究最紮實的基礎設施 doc 判成「沒做研究」,其中一份查出並修掉**三個 P0 資安漏洞** |

**規則**|
- 代理指標的結論**報出去之前,先抽樣看實際檔案**。三次裡有三次,看一眼就發現了。
- 只回報一筆結果的檢查**特別可疑** —— 那通常是「規則太鬆抓到雜訊」或「規則太緊只漏一條」。
- FMEA / 稽核的每一條都要先問:**「這個操作真的存在嗎」**。
  假的 P0 會排擠真的 P0 的注意力,而真的那條往往就在旁邊。

### 🔴 深度研究鐵則|站在巨人的肩膀 = 四站 + 親驗(2026-08-08,gl.md 一輪立三條)

「站在巨人的肩膀」原本三站(①自家 repo ②自己的相依套件 ③競品,[[pitfall-giants-shoulders-three-stops]]),
gl.md 總帳 M0 被決策方一句「有做深度研究嗎?」抓出兩個系統性缺口,補齊後 codify:

| # | 鐵則 | 由來 |
|---|---|---|
| 1 | **第④站|主管機關法規**:合規敏感模組(帳務 / 稅務 / 發票 / 個資 / 勞動 / 薪資)**必查一手法規**(law.moj.gov.tw 逐字 + 條號 + 查證日),不是只查競品。**只查競品不查法規是「範圍缺陷」不是「深度不足」** | gl.md 三家競品查完自我感覺完整,法規一查**兩條改設計**(商業會計法 §17 傳票法定三類 → schema;§38 十年保存 → 不變量)、三條 OQ 依據升級(§18 結轉法源 / 辦法無 hash 鏈明文 / 準則 §3 自訂科目法源)—— 法規不是背景知識,**是會改架構的一手來源** |
| 2 | **親驗才可承重**:研究 agent 回報的「逐字引用 + URL + 查證日」**格式合格 ≠ 可承重** —— 引句是 agent 抄的,抄錯 / 編造在委派層無法分辨。**M0 掛 APPROVED 前,承重句必須本人開檔抽驗**(下載原檔 grep / 本地 grep / 原文比對),**親驗行號記進 §0**。流程 = agent 研究 → 本人親驗 → 才裁定 | gl.md M0 v1.0 掛 APPROVED 時承重句零親驗([[feedback-verify-load-bearing-claims]] 第三種失效型)。補驗此次零反轉 —— 但零反轉不代表白做:它把「⭐ 有一手依據」從格式合格變成可承重 |
| 3 | **設計難題先列名**:design-ahead 深度停在架構裁定(不做實測)是合法的,但「**哪些設計難題存在**」必須在 M0 列出且逐條一手錨定(官方文件 / 權威實作逐字),實測項明列留 M1 —— 難題不列名,M1 就不會有人去測 | gl.md §7-bis(TigerBeetle hot-account / PG deferred trigger / append-only 聚合);dynamic-permissions 的 30 萬列實測範式是 M1 的樣板 |

**為何不加 CI 檢查**|「親驗過沒有」與「法規查了沒有」機器偵測不出(同〈代理指標〉教訓:
偵測規則只會產生假陰性)。防線是流程性的:M0 裁定前自問三站四站齊了沒、親驗行號在不在 §0。
