...
| **Auth + 租戶 context + 使用者身分**(Better Auth + Argon2id)| F-2 | 🚧 **M1 ✅**(認證引擎 + NestJS DI 接入,BETTER_AUTH_SECRET prod fail-fast)· **M2 ✅**(對映表 users + tenants.auth_org_id/parent_tenant_id + IdentityService 冪等 upsert/resolve)· §6-bis 登入分層 + 帳號治理決策 · **續 M3 AuthGuard**(getSession → tenantContext + 剝 header + 隔離測試;保留 TenantContext 介面 → services 零改;AUTH-8=場景 A 多 org 隔離,場景 B 僅預留 parent_tenant_id;三層 RBAC 屬 P0-4)| [foundation/auth.md](foundation/auth.md) |
