...
| **三層權限(授權層)** | P0-4a | 🚧 **後端 M1–M5 + FMEA SHIPPED(管理 UI 待做)**|表單級 RBAC(Guard 執法)+ 欄位級遮罩/寫白名單 + **role tree 部門繼承**〔OQ-1=C〕+ owner→admin 對映 + 權限管理 API;deny-by-default;縱深(RLS 仍為跨租戶最後防線);~65 tests 綠、§12 FMEA P0 全清。**剩:權限管理前端 UI**(獨立前端 commit)。記錄級延 P1-I | [R1/authz.md](R1/authz.md) |
