...
| **↳ 資源軸繼承(分類授權 + owner + 敏感旗標)** | P0-4a·uplift | 🚧 **M0 APPROVED — OQ-ARI-1..8 全裁定(2026-07-24;deep-research 錨定),待 M1**|解逐表列舉之維護膨脹(`O(表單)→O(分類)`):既有 `form_permissions` 重定位為**覆寫層**,其下補**分類授權層** `category_permissions` + owner 短路(`form_def.created_by`,**得資料動作 design 除外**)+ 敏感旗標(不吃繼承,admin-only)+ 租戶預設 profile(Salesforce OWD 式)+ 無權表三態(readable/鎖定 stub+申請/敏感隱藏)。純加法 migration 0008、惰性零回歸。**向上設計研究**(Notion "No more database accidents"/Salesforce/Drive/Purview)錨定 OQ,§10-bis。UI 對照 `docs/mockups/permissions-resource-inheritance.html` | [R1/authz-resource-inheritance.md](R1/authz-resource-inheritance.md) |
