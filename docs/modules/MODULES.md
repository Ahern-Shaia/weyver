...
| **視圖系統 + 集合(browse)視圖** | R1·UP-2 | 🚧 **M0 APPROVED — OQ-VL-1..7 全裁定(2026-07-25,全採建議),進 M1**|docs/27 §6 順序 2(承 workspace-ia):落地 D2「Ragic 語意、Airtable 骨架」—— 每表恆有**集合網格視圖**(複用 Glide record-grid-panel,套 view 選欄/篩選/排序)+ 與 Object Page **雙模式**(列表↔記錄,列表為預設)+ `view_def` 持久層(authz Tier-1 車道,非 RLS)**儲存檢視三態**(個人/共通/預設)+ facet 篩選(承 field-type-registry operator)+ 多鍵排序 + 快速搜尋(ILIKE textual)+ 批次刪除 + 匯出(view-scoped)。**OQ-VL-2 修正 docs/27 §3**:forcedFilter(固定篩選)非 view 屬性 → 移出歸 authz 軸(Airtable 明文 view filter 非安全邊界 / Teable Authority Matrix)。migration 0009。≈0.37 mo | [R1/views-list.md](R1/views-list.md) |
