...
| **選項顏色設定 UI(field-types-parity P1 解鎖)** | R1·UP-4c | ⏳ **M0 DRAFT — OQ-OC-1..7 待裁定(2026-07-28)**|`options.colors` 後端於 field-types-parity 已建、**前端從未有入口**(死參數);record-workbench-ui 狀態章已預備讀取。**§0 clean-room 證據推翻預設前提**:**Ragic 無 per-option 顏色**(其對應功能為**條件式格式**),per-option 色為 Airtable/Teable 範式 → 本模組是**增強而非遷移 parity**;兩競品實際色盤色數**文件未載**,任何色數皆為推測。**§0.2 攤開與 docs/14「裝飾配色一票否決」之正面衝突** → OQ-OC-1 交裁定(建議 A 只開 4 語意 tone)。另 OQ-OC-4 誠實指出:若求遷移 parity,**條件式格式(form-designer-2d P1 殘留)優先度應更高**。P0 = 逐項選項編輯器(順帶拆 422 行超標檔)+ 白名單安全渲染 + 孤兒清理。≈0.13 mo | [R1/option-colors.md](R1/option-colors.md) |
