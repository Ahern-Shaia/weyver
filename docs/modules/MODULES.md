...
| **影像處理(EXIF 剝除 / 縮圖 / HEIC)** | F-7 | ⏳ **M0 DRAFT v0.2 — OQ-IP-1..9 待裁定(2026-07-28)**|收斂 image-signature-fields S3(無縮圖)/ S4(EXIF 隱私)/ HEIC 不支援 + file-storage 縮圖殘留。**§0.1 本機實測推翻兩項前提**:(a) HEIC **不是加相依就能解** —— 我們的 sharp 0.34.5 預建版**無 HEVC 解碼器**(`heif: Error while loading plugin`),維護者已表態永不內含;(b) 縮圖**不需背景 job** —— 24MP→240px 實測僅 **28ms**,同步即可。另實測 **1.6MB 檔案宣告 149.8MP**,sharp 預設 pixel 上限**擋不住**(解碼約 450MB)→ 列 P0 解壓縮炸彈。**§0.2 研究**:授權可解(LGPL;decode-only 避開 x265 GPL)但**專利不可解** —— HEVC 專利池明文涵蓋雲端服務按 authorized-user 計費 → **HEIC 走前端轉檔**(iOS Safari 自動轉 JPEG)。真實照片 EXIF 實測含 GPS/Make/Model/DateTime,隱私風險已證實。**§0.3 競品證據**:Ragic 1 縮圖/預設高 120px/**永不放大**;Airtable **官方明載不改原檔**;Teable changelog **實證「縮圖忽略 EXIF orientation」之坑**與預生成派需自建重生;Dropbox 有**客戶端轉 JPG** 先例;**Partiful 2025-10 GPS 外洩事故**(兩日修補並回溯清洗)。**§0.4 實測推翻原建議**:重編碼是世代性失真(q95 反而**大 14%**)→ 改採**無損切除 metadata 段**(已驗證像素位元組不動、EXIF 消失),兼顧 Airtable「不改檔案」精神與消除 GPS。P0 = EXIF 無損剝除 + 同步縮圖(單一尺寸/永不放大)+ 炸彈防護 + 前端 HEIC 路徑。≈0.17 mo | [foundation/image-processing.md](foundation/image-processing.md) |
