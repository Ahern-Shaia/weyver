...
| **2D 表單設計器(form-designer-ui uplift)** | R1·UP-3 | 🚧 **M0 APPROVED — OQ-FD2-1..7 全裁定(2026-07-25,全採建議),進 M1**|docs/27 §6 順序 3(承 views-list):落地 D1「2D 格線畫布=填單畫面」—— 既有線性設計模式 uplift 成 Excel 式 **2D 格線畫布**(row/col/span 擺位、拖曳、合併)+ **靜態敘述/圖片元素** + **表單分段**(列範圍)+ **欄位設定核心**(預設值 17 變數 / 唯讀 / 隱藏 / placeholder / 說明)+ **設計草稿模型**(批次 Save + Ctrl+Z)。**核心洞見**:版面皆 layout metadata、與資料正交 → `form_def.layout` JSONB(migration 0010),form-engine-core DDL/DML 鏈**不動**(低風險)。OQ-FD2-2 定調:layout 走草稿、結構性 DDL 維持即時(Ragic 排除清單);靜態=layout 元素非 field_def(OQ-3);分段=列範圍(OQ-4);畫布 CSS grid + dnd-kit(OQ-5)。≈0.47 mo(四模組最大)| [R1/form-designer-2d.md](R1/form-designer-2d.md) |
