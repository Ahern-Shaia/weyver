...
| **條件式格式(form-designer-2d P1 解鎖)** | R1·UP-3b | ⏳ **M0 DRAFT — OQ-CF-1..8 待裁定(2026-07-28)**|form-designer-2d 之 P1 殘留。**§0 clean-room 證據:Ragic 與 Airtable 的模型相反,不可各取一半** —— Ragic=**表單級 + 欄位級著色 + 後者覆蓋 + 自由選色**,且表單頁與列表頁分開設定;Airtable=視圖級 + 整列著色 + 前者優先。Ragic-parity-first → 整套採 Ragic(OQ-CF-1/3/5/7)。**OQ-CF-2 需裁定**:Ragic 用自由選色,與 docs/14 §0.2 才剛立的「受控色盤非自由選色」衝突(建議沿用 12 tone,誠實代價=遷移色需映射)。**OQ-CF-8 明確排除條件式行為**(顯示/隱藏/唯讀/必填):條件式隱藏**不是安全邊界**、條件式必填只在前端即為裝飾 → 另立模組。P0 複用既有 filter 模型 + 純前端求值 + 存 form_def.layout → **零 migration 零新端點**。≈0.16 mo | [R1/conditional-format.md](R1/conditional-format.md) |
