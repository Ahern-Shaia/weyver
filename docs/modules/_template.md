...
## 0. 站在巨人的肩膀（**寫 §1 之前先寫這一節**）

> 🔴 **本節在 2026-08-03 補進範本。** 在此之前範本沒有研究節，
> 每份模組文件都是自己臨時加，結果 32 份裡有 14 份根本沒有
> （稽核見 `_audit/giants-shoulders-audit-A.md` / `-B.md`）。
>
> 判準見 `AGENTS.md`〈向上設計三條〉。**三站逐站列標題，沒查的那站寫「未查證」而不是省略** ——
> 省略看起來像查過了。

### 0.1 巨人一：自家 repo

既有 migration / service / schema / **上游 design doc**（`docs/15` `docs/16` `docs/25` `docs/27`）
已經裁定過什麼？有沒有已經寫好但**零 reader** 的欄位？

| 查了什麼 | 結果 |
|---|---|
| | |

### 0.2 巨人二：**自己的相依套件**（最常漏，且漏了最貴）

> 這一站沒有「做了功課」的外觀，卻是唯一能證偽「這件事要自己做」的一站。
> 查法：`find . -path "*<pkg>/dist/dts/*.d.ts"` 然後 grep 你要的 prop，
> **讀 doc comment 不要只看型別簽名** —— 註解裡常寫著「if unset X will not work」這種決定性資訊。
>
> 實例：`grid-paste` 原以為主要工程量是「解析剪貼簿 + 逐格寫入」，
> 查完發現 Glide **兩件都給了**，模組形狀當場改變。

| 套件＠版本 | 相關 API | 逐字註解 | 對本模組的意義 |
|---|---|---|---|
| | | | |

### 0.3 巨人三：競品

> **附出處 + 查證日期**，並區分「官方明說不支援」與「我沒找到」（後者寫未查證）。
> ⚠️ **clean-room**：讀原始碼前先確認授權 —— NocoDB / Directus 已非 OSS、
> Baserow enterprise 專有、Teable `apps/` 為 AGPL。非 MIT 者**只讀公開文件不看實作**
> （`AGENTS.md` 鐵則 5）。引用原始碼時**附授權識別碼**。
...
- [ ] **§1.3「不做的事」每一條也查過三站了嗎？**「不做」的決定同樣承重
      —— `grid-paste` §0.2 才寫完站②教訓，§1.3 就把 fill handle 判為「另立」而沒查套件，
      而該套件有現成的 `fillHandle` 與 `onFillPattern`
- [ ] 每個 OQ 的裁定**都有 §0 的哪一條在撐**？（列不出來的，就是「研究了但問錯問題」）
- [ ] 有沒有寫下「競品沒有 X」「各家都 Y」這類句型而**沒有逐字依據**？
- [ ] **禁寫「本模組無向上缺口」這類終局結論** —— 已兩度被下一輪推翻
- [ ] 🔴 **寫「待辦 / 應調整 / 暫緩」時,附當下的對碼證據(檔案:行號)** ——
      2026-08-03 一輪連續**四次**發現待辦其實早就做完了。待辦不會自己過期,
      而下一輪讀到它的人會當成現況。**「暫緩」尤其危險**:它不像「否決」有終局性,
      條件解除時沒有任何東西會提醒你 → 解除條件要寫成**可檢查的**
      (「等 X 落地」就在 X 的驗收清單裡加一行回頭改這裡)
...
| **M2** ... | ... | 0.05 mo | ⏳ |
| **M3** ... | ... | 0.02 mo | ⏳ |
| **M4** docs + smoke | doc → v1.0 + MODULES.md → ✅ + **`docs/25` 對應列狀態回填**(見下)| 0.02 mo | ⏳ |
| **M5** FMEA 收尾（R17）| 填 §12 失效場景反思（逐路徑 → 嚴重度 → 緩解）；P0 未緩解不得上 prod | 0.02 mo | ⏳ |

---

> **[P0] 研究即寫入 doc —— 不得只留在 task 或 commit message。**
> 任何為本模組做的競品 / 規範 / 實測研究,**當下就寫進本檔 §0 證據段**,並附**可點擊的來源連結**。
>
> **為什麼是硬規定**|2026-07-28 追溯稽核時發現:Tier 1/2 六個模組的研究成果全落在
> task 描述(session 內、不進 repo)、commit message(在 git 但難查)與程式碼註解(只涵蓋修掉的部分)
> —— **六份 doc 的證據段是 0**。最容易永久遺失的是**來源 URL**(每路研究 15–25 條),
> 事後補寫時只能從對話上下文搶救。研究的價值有一半在「日後能重新查證」,不寫下來等於白做。
>
> **追溯補寫時**用 `## 0-bis. 追溯稽核(日期)` 標明是事後補,不要偽裝成當初就有。
>
> **[P0] 寫完必逐條回對原始研究產出。** 2026-07-28~29 連三批補寫,**每一批都漏**:
> Tier 1 漏 3 處(負面發現 / 驗證清單 / 可實作寫法)· Tier 2 漏 1 處(推斷規則完整規格被壓成摘要)
> · Tier 3 漏 3 處(NN/g task-based 結論 / Fiori 表格量級階梯 / 三種版型的取捨理由)。
> **共同形狀:摘要時把「可直接照做的規格」與「負面發現」丟掉** —— 那恰是日後最有用的兩類。
> 逐條抽查是寫 §0 的**必要步驟**,不是可選。

---

> **[P0] M4 必做 —— `docs/25` 狀態回填。** 模組 SHIPPED 後,回 `docs/25 功能完整對照清單` 把本模組涵蓋到的子功能列改成 ✅ 或 🟡,**🟡 必須註明「已交付」與「未起」的界線在哪**(例:「⌘K 僅搜表單名 ✅;跨表記錄全文搜尋未起」),並重算該模組區塊與 A–I 合計的覆蓋率。
>
> **為什麼是硬規定**|docs/25 是「仿 Ragic 進度」的唯一權威來源,但它**不會自己更新**。2026-07-28 校對時發現狀態欄停在 07-24、只落後四天就已嚴重失真(B 段仍記「19 型已建 / 2D 版面未起」,實為 28 型 + 2D 設計器已 SHIPPED),導致無法回答「完成度多少」而必須整份重查。**不回填 = 下次又要重查一遍。**
