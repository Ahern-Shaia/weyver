...
>
> ✅ **已裁定 2026-08-03**(2026-08-04 回填,`_audit/giants-shoulders-audit-C.md` §3-5 ——
> `AGENTS.md` §5-bis 逐字寫「`pivot-and-charts` §0 同此處置,**已標註**」而本檔一直沒標,
> 是**規則文件過度宣稱**,屬 `pitfall_rule_without_check_always_drifts` 的形態)。
>
> **裁定內容(見 `AGENTS.md` §5-bis)**|**既成引用保留,往後從嚴。**
> 理由是逐項驗過而非憑感覺:採用的四項全是**架構想法**(色彩兩層制 / 由 base 以 `color-mix` 推導 /
> `--icon-*` 對齊文字階 / 禁 raw hex 升 CI),**沒有任何 Metabase 的值或程式碼進入我方 code**;
> 且 `color-literal.test.ts` 已在 CI 擋 raw hex,`tokens.css` 為唯一豁免 ——
> 那同時也是「外部色值混不進來」的守衛。
> **自 2026-08-03 起,AGPL 與非 OSS 來源一律只讀公開文件**;本檔與 `docs/28` 為該日之前的
> **明示例外**,範圍限已記錄的觀察,不擴張、不作為往後直讀原始碼的先例。
...
> §0 引用 Metabase 原始碼者為本檔既有內容,已依 §0 導言之 2026-08-03 裁定結案(既成保留、往後從嚴)。
