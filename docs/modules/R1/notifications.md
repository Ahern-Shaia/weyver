...
> ✅ **狀態:SHIPPED v1.0(2026-07-28)** — M1–M5 完成。簽核終於能通知下一關的人(本模組存在的理由)。
> api 391 綠 / web e2e 44 綠。migration 0018–0020。
> **P0 範圍**|站內通知中心 + Email 通道 + 三軸訂閱設定 + 簽核接通 + 風暴防護 + 同記錄去抖動 + 抑制清單。
> **不含**(各有理由,見 §1.2 / §4.6)|提醒(需 cron 地基)· LINE 等外部通道(自成模組)· 註解與 @提及 · 跨記錄 digest · 手機推播。
...
> **出處(2026-08-06 補;逐條由本專案開本機鏡像原文覆核)**
> [`doc-user/12` 通知設定](https://www.ragic.com/intl/zh-TW/doc-user/12) ·
> [`doc/5` 提醒](https://www.ragic.com/intl/zh-TW/doc/5) ·
> [`doc/96` 排程管理](https://www.ragic.com/intl/zh-TW/doc/96) ·
> [`doc/94` 寄出自訂 E-mail](https://www.ragic.com/intl/zh-TW/doc/94) ·
> [`doc/32` 存取權限層級](https://www.ragic.com/intl/zh-TW/doc/32)

**[`doc-user/12`](https://www.ragic.com/intl/zh-TW/doc-user/12) 通知設定** —— 三層結構,**全部由終端使用者自己設定**:
...
**「跟我相關」的官方定義**([`doc-user/12`](https://www.ragic.com/intl/zh-TW/doc-user/12) 逐字):
...
> **關鍵設計證據**([`doc-user/12`](https://www.ragic.com/intl/zh-TW/doc-user/12) 逐字,
> 本專案 2026-08-06 開本機鏡像原文覆核):
> > 注意: 1. **透過大量修改或是匯入的資料即使符合條件也不會寄送通知。**
>
> 這是刻意的**通知風暴防護**,不是遺漏。

**[`doc/5`](https://www.ragic.com/intl/zh-TW/doc/5) 提醒(Reminder)** —— 設計者端、**宣告式**、日期驅動:
- 前提:表單有**日期欄位**
- 設定:對指定日期欄位,**幾天前或後**提醒
...
**[`doc/96`](https://www.ragic.com/intl/zh-TW/doc/96) 排程管理**([逐字](https://www.ragic.com/intl/zh-TW/doc/96):「要立刻執行某項排程,可以直接點**馬上執行**」)|集中設定各功能執行時間(提醒 / **重寄逾期簽核通知信** / 資料封存 / 表單填寫提醒 / 定期匯入 / SQL 同步 / Daily Workflow / 定期寄出報表);同一功能可多組排程;提供「馬上執行」。

**[`doc/94`](https://www.ragic.com/intl/zh-TW/doc/94) 寄出自訂 E-mail**(逐字:「在「選擇名稱」模式下,可選擇**執行動作按鈕的使用者名稱,或公司設定的寄件者名稱**」;「**參數無法套用於單一子表格欄位**,只能帶出整個子表格的內容」)|動作按鈕型,可**儲存資料後自動寄出**;寄件者名稱可選「執行按鈕的使用者」或「公司設定的寄件者」;有歷史紀錄。另有 `doc-user/5.4` **信件紀錄**。
...
## 11-ter. WhatsApp Business 通道(2026-08-05)

Meta Cloud API(`graph.facebook.com/v21.0/{phone-number-id}/messages` + Bearer token)。
H 段最後一個 ⬜ 的通道。

🔴 **與其他通道有一個本質差異,設定頁必須講出來**|WhatsApp 只允許在使用者主動來訊後的
**24 小時服務窗**內自由發文字;窗外只能送**事先核准的範本訊息**。我方送的是純文字通知,
故實務上**只在對方近期回過訊息時送得出去**。這不是我方的限制,是平台規則 ——
不講的話,設定者會以為是自己填錯而反覆重試。`secretHint` 已明載,並有測試釘住那句話。

⚠️ **誠實聲明:未做端到端發送驗證。** 需要 Meta 的 access token 與 phone-number-id,
本機無法取得。目前有的是:通道註冊表單元測(host 白名單防後綴伎倆 / 24 小時提示 /
必填欄位)+ 瀏覽器實走確認它出現在設定頁清單。
**「設定得起來」已驗,「送得出去」未驗** —— 兩者不要混為一談。

🔴 **加這個通道時踩到、也修掉的一件事**|後端 `ChannelId` 加了、前端 `use-channels.ts`
的 `z.enum` 忘了 → 解析失敗 → **整個通道設定頁掛掉**,畫面上只剩一段 zod 錯誤。
這與 `audit-D §2.2` 的簽核 `fieldRef` 是同一形狀。處置分兩步:
(a) 加 `channel-mirror.test.ts` 比對前後端清單(已驗過拿掉會紅);
(b) 型別接著抓到**第三份**複本(`channel-card.tsx` 的 `SPEC`,其註解自己寫著
「兩邊都改才算改完」)—— **那份直接刪掉改由後端供給**,
而不是再加一道守衛。**一份資料只有一個來源時,「兩邊都改」這個問題就不存在了。**

---

## 12. 失效場景反思(FMEA)— ✅ M5 確認(2026-07-28)

| # | 場景 | 緩解 | Sev | 狀態 |
|---|---|---|---|---|
| N1 | **通知風暴**:匯入 5000 筆 → 5000 封信 | 事件由 service 層**顯式 emit**(OQ-NT-3);批次路徑(Excel 匯入 / bulk)不呼叫 emit → 不通知為**預設行為而非例外** | **P0** | ✅ 已緩解 |
| N2 | **跨租戶洩漏** | `notification` 等四表皆帶 `tenant_id` + RLS FORCE + policy;整合測斷言 B 租戶使用者收不到 A 的通知 | **P0** | ✅ 已緩解 |
| N3 | **繞過欄位級權限**:通知含收件人無權見的欄 | `safeTitle()` 只用表單名 + 記錄編號,**型別上不接受欄位值**;Email 內文同規則。整合測以「金額」為首欄斷言不含其值;e2e 於 DOM 斷言 | **P0** | ✅ 已緩解 |
| N14 | 🔴 **記錄標題本身洩漏受保護欄位**(`titleOf()` 取 `fields[0]`)| 通知**不復用 `titleOf()`**;`safeTitle()` 為唯一產生處 | **P0** | ✅ 已緩解 |
| N15 | 硬退 / 投訴未處理 → 網域信譽崩壞 → 全體租戶通知進垃圾信 | `email_suppression` 表 + **寄送前必查**;5xx 立即永久 suppress;依 SMTP 狀態碼分類不解析文字。整合測斷言已抑制位址不再寄 | **P0** | ✅ 已緩解 |
| N4 | 寄件失敗靜默丟失 | `notification_delivery` 狀態機(pending/sent/failed/skipped)+ `attempts` + `last_error`;軟失敗指數退避,達 5 次升硬失敗 | P1 | ✅ 已緩解 |
| N5 | 重複寄送(多實例同時掃) | `FOR UPDATE SKIP LOCKED` + 交易範圍 advisory lock | P1 | ✅ 已緩解 |
| N6 | 使用者關閉全域通知後仍收到簽核通知 | 總開關關閉即不發送;**唯一例外為簽核逾期**(裁定 ④),且**設定頁明白告知**(靜默的例外會讓使用者以為設定壞了)。整合測與 e2e 皆斷言 | P1 | ✅ 已緩解 |
| N7 | 離職 / 停用使用者仍收到通知 | 收件人解析檢查 `users.deleted_at`;整合測斷言 | P1 | ✅ 已緩解 |
| N16 | 同一記錄連續編輯 → 通知風暴 → 使用者關掉全部通知 | per(收件人 + 記錄)去抖動視窗 3 分鐘(承 Jira),**簽核類 bypass 立即送**;合併時一封信涵蓋 N 則。整合測斷言簽核立即到期、一般異動排到視窗後 | P1 | ✅ 已緩解 |
| N17 | `LISTEN/NOTIFY` 於 PgBouncer tx mode 不可用 / RLS FORCE 擋 worker 跨租戶掃描 | **不使用 LISTEN/NOTIFY**,改輪詢;跨租戶掃描走特權車道(沿用 F-8 `UsageService` 既有解)。M1 第一件事即確認 | P1 | ✅ 已緩解 |
| N8 | Email 落入垃圾信 / 網域信譽受損 | 走 relay 共用 IP pool(不自架 SMTP);`List-Unsubscribe` 介面已備 | P1 | ⚠️ **部分**|**SPF/DKIM/DMARC/PTR/專屬寄件子網域/VERP bounce 收取/Postmaster Tools 監控 皆為部署期設定,尚未進行** → 見 §12.1 |
| N12 | 群組廣播洩漏(LINE 群組含無權者) | — | **P0**(群組上線時)| ⏸ 群組不在 P0;§4.6 已立設計約束 |
| N13 | 群組失效靜默 | — | P1 | ⏸ 同上 |
| N9 | 通知累積無上限 → 表膨脹 | — | P2 | ⚠️ **未做**|Discourse 式上限保留(每人最新 N 筆)尚未實作。pilot 量體不緊迫,但**應在客戶數成長前補** |
| N10/N11 | 通道綁定錯人 / on-prem 收不到 webhook | — | — | ⏸ 屬 LINE 模組 |

**P0 全數緩解**。殘留 3 項:N8 部署期設定(**pilot 前必做,否則通知等於沒送出**)· N9 保留策略 · 群組相關(不在 P0)。

### 12.1 上線前必做(不是程式碼,是部署)

依 §0.4.5 研究,以下**不做就會進垃圾信或被 550 拒收**,且與程式碼無關:

1. 專屬寄件子網域 + **另一個**專屬 bounce 子網域
2. SPF **且** DKIM(≥1024-bit)+ DMARC `p=none`;From 與 SPF 或 DKIM 對齊
3. PTR 正反解互相匹配 + 對外一律 TLS
4. **VERP Return-Path + DSN 解析**接回 `email_suppression`(目前只有硬退時由寄送端寫入,**尚無 bounce 收取管線**)
5. Postmaster Tools 驗證網域 + `Feedback-ID` 標頭;垃圾率日監控
6. 走 relay 共用 IP pool,**不要在雲主機 IP 直寄 port 25**

> **量級不是理由**|bulk sender 門檻為 24 小時內 5,000 封至個人 Gmail,**子網域併入母網域計算且身分一旦取得永不失效**。照 bulk 規格建,不要照現況量級建。

### 12.2 實作期發現(記錄以免重蹈)

- **指名收件人被角色過濾**|原以 `role_members` 過濾所有收件人,會**靜默丟掉沒有任何角色的人** —— 使用者可憑 owner 短路或租戶預設權限建單送簽,卻永遠收不到結果。指名來源本身已 tenant-scoped,該過濾多餘。
- **`scope_id` 為 NULL 使唯一索引失效**|SQL 中 `NULL ≠ NULL`,`ON CONFLICT` 永不觸發 → 租戶層偏好每次修改都**新增一列**,解析取到過期值。改 `NOT NULL DEFAULT 0`。
- **應用時鐘 vs 資料庫時鐘**|去抖動排程用 `Date.now()` 寫入、到期判斷用 PG `now()`,時鐘偏差使通知**永不到期**。改由 DB 單一時鐘來源計算。
- **通道偏好誤做成整批共用**|一個旗標決定全體收件人是否收 Email,實為逐人設定。
- **`engineFetch` 對無 body 的 POST 仍宣告 `content-type: application/json`**|Fastify 直接回 500。屬共用層缺陷,順修。
...
| 日期 | 版本 | 變更 | 作者 |
|---|---|---|---|
| 2026-07-28 | **v1.0 SHIPPED** | M1–M5 完成。**M1** 四表(notification / delivery / pref / setting,皆 RLS)+ 有序 enum 層級 + 分類資源軸繼承 + 顯式 emit 風暴防護 + `safeTitle` + 簽核接通;**M2** 收件匣端點 + 鈴鐺面板(Discourse 式部分索引算未讀);**M3** Email 通道(relay / 抑制清單 / 退避重試 / 同記錄去抖動 / `References` threading);**M4** 三軸設定頁;**M5** e2e 3 測 + FMEA。**實作期揪出五個缺陷**(見 §12.2):指名收件人被角色過濾而丟掉無角色者 · `scope_id` NULL 使唯一索引失效致偏好每次新增一列 · 應用時鐘與 DB 時鐘不一致使去抖動永不到期 · 通道偏好誤做成整批共用 · `engineFetch` 無 body POST 仍宣告 content-type 致 500。api 391 綠 / e2e 44 綠。migration 0018–0020。**未做**:提醒(待 cron)· LINE 等通道 · 註解 @提及 · 跨記錄 digest · N8 送達率部署設定(**pilot 前必做**)· N9 保留策略 | Claude Code |
