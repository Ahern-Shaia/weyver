...
### 0.2 巨人二:自己的相依套件(2026-08-04 回填,`_audit/giants-shoulders-audit-C.md` §3-1)

🔴 **這一節本來不存在。** 下列三件事全是本模組**實測踩出來的**,卻只寫在變更紀錄裡 ——
而變更紀錄是給讀歷史的人看的,不是給下一個做匯出的人看的。**同一個檔案已經踩過三次。**

| 套件＠版本 | 逐字 / 實測 | 對本模組(與下一個模組)的意義 |
|---|---|---|
| `archiver@7` + `@types/archiver@8` | 型別宣告 `export class ZipArchive`,但**執行期只有掛著 `create` 的函式** | `tsc` 全綠、一跑就 `not a constructor`。**型別檔與執行期不一致的套件,型別檢查給不了任何保證** |
| 同上 | `progress` 事件的 `fs.processedBytes` **只涵蓋檔案系統來源的 entry** | 我方全部是 stream → 該值**恆為 0**,掛在它上面的大小上限**形同虛設**。改為自己累加 |
| 同上 | `archiver` 是 **CJS**,而本專案是 ESM | `import * as m` 之後 `m.create` 在 **vitest 下存在、在 tsx(dev/prod 實際執行方式)下是 undefined**。症狀是**單元測試 9 條全綠、瀏覽器一按就失敗**,錯誤只在 dev server 的 stderr。改用 `createRequire`(Node 原生 CJS 載入,不經任何轉換器的 interop 詮釋)後兩個執行環境才一致 |

👉 **三次的共同形狀:單元測試環境與實際執行環境不同,而測試只證明了前者。**
本模組因此把驗收標準改成「e2e 必須驗到 `ready`,不能只驗畫面」——
這一類缺陷**只有跑在 tsx 上才攔得住**。

⚠️ 其餘相依(`xlsx` / storage driver)**未做同等查證**。
...
| E8 | **每日 10 次上限被自動化測試消耗** —— 同一天多跑幾次 e2e 就會紅;客戶遷移期反覆試也可能撞到 | ✅ **測試面已解(2026-08-04)**:`e2e/global-setup.ts` 每輪歸零 dev 租戶當日的 `export_job`。⚠️ 那是 `field-label-a11y` §7-bis 順帶修的,**該處有回填、這裡沒有** —— 跨模組的順手修最容易只留下一半紀錄。**產品面殘留**:額度仍未區分自動化與人為,亦無「剩幾次」的提示 | P1 | 🟡 部分 |
