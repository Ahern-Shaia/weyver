...
## 14. IA 第二階段(2026-08-04)—— 面板搬進表單層

承 `docs/33` OQ-IA-3 = A。第一階段(commit 7997799)只做了**入口聚合 + 深連**;
面板本身仍住在設定中心。第二階段把 per-form 的那一半真正搬過來。
...
**發現 1|Ragic 的 per-form 對外設定就開在列表頁的「工具」,而且刻意做成不必離開**

> 「在列表頁的 **工具** 中找到 **發佈到網路**。」
> 「如果想要停止發佈,同樣可以這個視窗點選 …… **如此一來就不需要進到修改設計中調整**。」
> —— Ragic 設計手冊 `doc/71`「發佈到網路」(查證 2026-08-04)

⚠️ **範圍誠實**|Ragic 的「發佈到網路」是把資料**發佈成可下載的 Excel/JSON**,
與我們的「公開表單」(匿名**填寫**)不是同一個功能。
可承重的只有**落點與形態**:per-form 的對外設定開在列表頁「工具」、以**視窗就地**完成。
不可承重的是功能對等。

**發現 2|🔴 Ragic 的 per-form 通知**確實**存在,而且入口就在表單的工具選單**

> 「**表單個別通知** —— 在表單的 **工具** 選單中的 **同步與通知** 選擇 **通知設定**。」
> —— Ragic 使用手冊 `doc-user/12`「通知設定」(查證 2026-08-04)

同一份文件同時記載,個人設定裡另有「**頁籤個別設定**」:
> 「選擇想要設定通知的表單所在頁籤。針對頁籤中的**個別表單**設定是否要收到以下四種情況的通知」

**這是同一筆設定的兩個入口。** OQ-IA-3 當初否決選項 B(雙入口)的理由是
「改哪個才算數會變成客訴來源」—— 那個理由**只在兩處各存一份時成立**。
Ragic 的兩個入口寫的是同一列,不存在歧義。

🔴 **據此修正 OQ-IA-3 的執行細節**(裁定 A 不變,做法變):
設定中心那一半**不是刪掉**,而是從「**選一張表單的下拉**」改成「**逐表單列出的清單**」。
`docs/33 §2.1` 抱怨的一直是**下拉**(離開表單後還要把同一張表再選一次),
不是「設定中心不該有 per-form 這一層」——
一次調整我對多張表單的訂閱,是清單才做得到、下拉做不到的事。

**發現 3|🔴 兩個現行入口的權限閘門都是錯的**(第一階段留下的)

`form-tools-menu.tsx` 把「公開表單設定」與「此表單的通知設定」**一起**放在 `canDesign` 之後,
依據是 `docs/33` P4「搬到表單層之後必須改由該表單的 `design` 動作權把關」。**P4 本身錯了。**

| 入口 | 舊閘門 | 正確閘門 | 一手依據 |
|---|---|---|---|
| 公開表單設定 | `design` | **`isAdmin`** | 後端 `public-form-admin.controller.ts:48` 逐字:「開放一張表單給外部人填寫是**租戶級的安全決定,不是表單級功能** → 限 admin」。有 `design` 沒 admin 的人**看得到、按不動** —— 正是我們剛為 `canDesign` 修掉的「畫面說謊」,在上一層又犯一次 |
| 此表單的通知 | `design` | **不設閘門**(進得到這個表單就能設) | `notification_pref` 帶 `actor_id`(`db/schema.ts:858`)= **個人訂閱**,不是表單設定。Ragic 把它寫在**使用手冊**而非設計手冊。用 `design` 擋住,等於一般使用者不能管自己的通知 |

**共同形狀**|P4 把「搬動不得放寬權限」正確地寫成原則,卻**推導錯了具體閘門** ——
它假設「原本在 admin 頁面裡」就等於「該用設計權」。
真正的判準是**這件事的粒度與擁有者**:租戶級安全決定 → admin;個人訂閱 → 本人。

### 14.2 落地

| 面 | 動作 |
|---|---|
| 表單層 | 新增 `form-share-panel`(建立分享 + **本表**分享清單 + **本表**待審)與 `form-notify-panel`(此表單的通知層級)。沿用 `ImportPanel` 的全接管形態 —— 同樣由「工具」開啟,不新增第三種面板語彙 |
| 設定中心 · 公開表單 | 移除「選一張表單再建立」的建立區;保留**跨表單總覽**(關閉 / 重新開放 —— 租戶級的資安視角)與**收件匣**。空狀態指路到表單 |
| 設定中心 · 通知 | 表單下拉 → **逐表單清單**(對齊 Ragic 頁籤個別設定)。租戶級三軸原樣保留 |
| 後端 | 補 `DELETE /api/notifications/prefs` —— 現況設了就**回不到繼承**,而畫面上寫著「未設定則繼承上層」。單向的設定是陷阱 |
| 閘門 | 依 §14.1 發現 3 改正 |

**沒有做的**|per-form 分享 / 收件匣的**伺服器端**表單過濾。
兩個 DTO 都已帶 `formId`(`schemas.ts:601`、`:615`),清單本身受 admin 閘門保護且有上限,
在前端過濾不會多洩漏任何東西 —— 為此新增一組查詢參數是無收益的 API 面。
...
| 2026-08-04 | v1.1 | **IA 第二階段**(§14):公開表單 / 通知的 per-form 面板搬進表單層;設定中心的表單下拉改逐表單清單;補 `DELETE /notifications/prefs`(單向設定是陷阱)。**🔴 一手研究翻掉兩件事**:Ragic 的 per-form 通知**入口就在表單工具選單**且與個人設定為同一筆的兩個入口(故 OQ-IA-3 的「刪掉設定中心那一半」做法有誤,改為下拉→清單);`docs/33` P4 推導出的權限閘門**兩個都錯**(公開表單應為 admin 非 design、個人通知不該設閘門) | Claude Code |
