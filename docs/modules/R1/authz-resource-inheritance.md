...
| **OQ-ARI-9** 🆕 **2026-08-03** | **分類管理員角色** | A. **引入**:對指定分類內的表單具 design + 使用者管理<br>B. 不做,design 權維持「租戶 admin」或「逐表授權」兩種 | ✅ **A(裁定 2026-08-03)** — 一手補查(§10-ter)發現的 **Ragic-parity 真缺口**:Ragic 以**群組頁籤 + 群組管理員**提供了「把 design 權下放到**容器層**給指定人」的路徑,本模組沒有對應物。<br>**不做的代價不是少一個功能,是撞命門**:大型租戶的 design 權會全部壓在租戶 admin 身上 —— 而「要找管理員才能改表單」正是 `AGENTS.md` 第一約束所禁的「設定外包」的變體。<br>🔴 **採納時的硬約束(Ragic 官方自列的風險,§10-ter.2 末段)**:容器層設計權會讓該管理員**經動作按鈕影響其無權限的目標表單** → **動作 / 拋轉路徑必須補目標表單的權限檢查**,否則這個角色就是提權管道。<br>✅ **2026-08-03 落地,但形狀與預期不同**:對碼後發現**容器層授 `design` 本來就走得通** —— 分類繼承(層 3)不過濾任何動作,`setCategoryActions` 也不擋 `design`。**不需要新角色型別**,「分類管理員」= 在分類層授予 `design` 的角色。<br>硬約束**已滿足**:`button.service.ts:145` 檢查目標表單的 `create` 權,簽核 `onComplete` 亦帶核准者權限走。<br>🔴 **真正缺的在前端**:`canDesign` 寫死 `true`、`/me capabilities` 從未做 → 被授權的人**在畫面上看不到設計入口**,功能形同不存在。已補 `GET /api/authz/me`。<br>行為測試已釘:容器層授 design 走得通 / 敏感表不吃繼承 / 逐表覆寫蓋掉分類。**日後若有人為了收緊而在繼承層加動作過濾,一加就會把這個角色靜默廢掉。**<br>殘留:「使用者管理」那一半(分類管理員管成員)仍為租戶 admin 專屬,列 P1 |
