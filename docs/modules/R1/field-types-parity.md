...
### 🔴 第六個出口:全文搜尋索引(2026-08-06)

上一段寫「新增出口時那一條會紅」。**但搜尋索引不是端點,它不在那條清單上。**

`SEARCHABLE` 由型別登錄表推導 —— `dbFieldType === "text"` 且非虛擬欄。
`textMask` 兩個條件都符合,於是**真的身分證字號被寫進 `search_doc.value_text`**。
沒有任何端點回傳它,但搜尋「A1234」搜得到那筆記錄,**搜尋成了值的預言機**。

補法刻意不是在搜尋那邊列一份黑名單(那就是第二份鏡射,必然漂移),
而是把 `sensitive` 標在**型別定義上**,`SEARCHABLE` 多一個 `def.sensitive !== true`——
往後新增的敏感型別自動繼承,不必有人記得回來改。

「值只要有第二個出口就會漏」至此**第六次**。
教訓更新:**出口不只是端點**,還包括索引 / 快取 / 匯出 / 通知這類「值的副本」。

### v1.7 收尾(2026-08-06)

| 項目 | 處置 |
|---|---|
| 設計器 UI | ✅ `text-mask-panel.tsx`。遮罩方式 / 位數 / 可揭露群組;範例用**與伺服器同一支** `maskText`。⚠️ 獨立檔案:`field-settings.tsx` 已 624 行(紅線 400) |
| 沒選群組的後果 | 面板明寫「只有系統管理員能按眼睛」。不寫的話設計者會以為「沒選 = 大家都能看」,而實際上**相反** |
| 匯出 / PDF | ✅ 查證後**兩者都已覆蓋**:前端 CSV 走 `/records`、租戶匯出走 `listRecords`、PDF 走 `getRecord` **且帶請求者的 actor 與權限**(不是系統身分)。三者都在既有的出口測試上 |
| 全庫修改紀錄 | ✅ 併入出口清單。它只回「動了哪些欄」不回值,但讀的是同一張表 —— 一起釘住 |
| `maskTextFields` 的型別 | 🟡 原本就地寫窄物件而**漏了 `keep`**。執行期沒事(整個 options 照傳)但那是巧合;改用 `TextMaskOptions`,免得有人照型別 destructure 後位數靜默回預設 |
