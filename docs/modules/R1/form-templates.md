...
| | |
|---|---|
| 狀態 | ✅ **SHIPPED v1.2(2026-08-07;M6 安裝紀錄 + M7 僅新增式更新)** · v1.0(2026-08-03;M1–M5)|OQ-TPL-1..10 全採建議。⚠️ 視圖與條件式格式**刻意未帶**(見 §4-ter,是「先不做」不是「做不到」)|
| 建立 | 2026-08-03 |
...
> 🔴 **2026-08-07 更正:這個承重結論是從 Airtable 一個資料點過度一般化,而 Ragic 就是反例。**
> `doc-kb/204` 逐字:「若需要對免費範本表單進行『**修改設計**』(如新增欄位、修改欄位名稱、
> 變更欄位種類、刪除欄位等),則會計算在『**客製化表單**』額度內」
> ([來源](https://www.ragic.com/intl/zh-TW/doc-kb/204),2026-08-07 由本地鏡像逐字覆核)——
> **Ragic 明確允許改**,而 `doc/37` 同時明確有「更新」入口。兩者並存。
>
> 所以「不可兼得」這個**全稱命題不成立**。Airtable 的 managed app 是「雙向同步 + global schema」
> 那一種更新形態,那當然要鎖 —— 但那不是唯一形態。
>
> ⚠️ **仍然未查證的**:Ragic 更新時**對已改過的表做什麼**。
> 2026-08-07 對本地鏡像全庫搜過(`更新模組` / `模組更新` / `覆蓋` / `重新安裝`),
> 只找得到入口的存在,找不到衝突處理的語意。**不得據此宣稱「Ragic 證明不鎖也能安全更新」。**
>
> 教訓對應 [[feedback-verify-load-bearing-claims]]:**方向性結論最易反轉,而反轉後論述仍通順不自曝。**
> 原文讀起來完全合理,因為它引的 Airtable 逐字是真的 —— 錯在把一家的形態當成物理定律。
...
- ⚠️ **範本更新已套用的實例** —— v1.0 列為不做,理由是 §0.5 的「要能更新就必須先鎖」。
  **該理由已於 2026-08-07 被自家查證推翻**(Ragic 允許改且有更新入口)。
  M6 已補上更新的**前提**(安裝紀錄:知道裝了什麼、哪一版、對到哪些 formId);
  **2026-08-07 裁定 OQ-TPL-11 = B 並落地(M7)**:僅新增式更新。
...
| **OQ-TPL-11** 🆕 | **更新已套用的實例要做到哪** | A. 只偵測「有新版」,不提供套用<br>B. ✅ **僅新增式更新(2026-08-07 決策方裁定 B,已落地 M7)**:只建 pack 有而租戶沒有的表 / 欄位,**絕不改名、不改型別、不刪除**,套用前給差異預覽<br>C. Airtable 式受管更新(鎖住範本部分) | **待裁定** —— 建議 **B**。C 已排除(鎖與第一約束相衝)。A 是誠實的最小值但等於把問題丟回給使用者。<br>**B 的理由**:它**不可能弄壞使用者的東西**,因為它從不觸碰既有物件 —— 定位風險為零,剩下的是正確性問題(使用者把欄位改名 → 我們會再加一個同義欄位),而那用**差異預覽 + 明確確認**處理。<br>⚠️ **證據強度要誠實**:推翻「必須先鎖」有一手依據(Ragic 允許改 + 有更新入口),但「僅新增式」這個具體形態**是推理不是查證** —— Ragic 更新時對已改過的表做什麼仍未查證。故列為待裁定而非逕行實作 |
...
| 2026-08-07 | v1.2 | **M7 僅新增式更新落地**(OQ-TPL-11 = B,決策方裁定)。<br>**唯一的不變量:只新增,絕不改名、不改型別、不刪除、不動版面、不碰資料。** 它因此**不可能弄壞使用者的東西** —— 定位風險為零(從不觸碰既有物件),剩下的是正確性問題。<br>**預覽與套用走同一段 `plan()`** —— 兩段各算一次的話,「預覽講的」和「實際做的」一定會漂,那比不給預覽更糟。<br>🔴 **偵測不到的那一種錯,講出來**:比對是**用欄位名**(ref 只對到表,對不到欄位),使用者把範本欄位改過名我們就認不出來,會多加一個同義欄位。舊版 pack 定義不在程式裡 → **偵測不了**,所以 `caveat` 是回應的必要欄位,而且逐一列出要加哪些欄位讓人自己判斷。<br>**使用者刪掉的表列 skipped 不重建** —— 那是他的決定,「更新」把刪掉的東西變回來是驚嚇不是服務。<br>`0066` 加 `kind` + `supersedes_install_id`:少了它,UI 分不出「你有 2 份請購申請」和「你更新過一次」。<br>測試守的是不變量而非功能:pack 拿掉的欄位**必須還在**、既有欄位 `cellValueType`/`dbFieldType` 原封不動、預覽連跑三次表數不變、`UpdatePlan` 型別上**不存在** `removedFields`/`renamedFields`(有人日後想加,那條斷言會逼他重新想過)。20/20 綠。 | Claude Code |
| 2026-08-07 | v1.1 | **M6 安裝紀錄落地** —— 補的是 **OQ-TPL-6=C 裁定了但沒做**的東西。<br>🔴 查驗起點:`version` 在 `packs.ts` 寫了 **8 次、全庫 reader 為 0**,也沒有任何「這個租戶裝過哪些 pack」的表 —— 正是 [[pitfall-unread-schema-field-drift]],而且是自己三天前寫的欄位。OQ-TPL-6 當時就標了「⚠️ 這是『現在不記以後補不回來』的那種決定」。<br>**兩張表**(`template_installs` + `template_install_forms`)而不是一個 JSONB 欄位:安裝紀錄會出現在範本庫每一次列表查詢裡,而 [[pitfall-serialized-column-breaks-whole-list]] 記著「一列壞掉炸整個 list」。<br>**刻意沒有 (tenant, key) 唯一約束** —— M4 已確立「同一個範本再套一份」是合法意圖;唯一約束會把那條路堵死,也會抹掉安裝史。同理 GRANT **不給 UPDATE**:更新到 v1.1 是**再插一列**,不是改寫舊列。<br>`form_id` 走 `ON DELETE SET NULL` + 另存 `form_name`:表被清掉時,那一欄是唯一還講得出「原本是哪一張」的東西。<br>🔴 **順手修掉 #55 的一個實例**:`templates.integration.test.ts` 原本拿 **superuser 當 app 車道**,新表的 RLS 與 grant 在那裡完全不會被執法([[pitfall-privileged-lane-masks-security]])。改用 `app_login`(NOSUPERUSER NOBYPASSRLS)後,grant 缺一個第一條測試就會紅。<br>**§0.5 的承重結論被推翻**(見該節更正欄),`版本比較` 另立 `compareVersion` 不引 semver。13/13 綠 + dev 實走 round-trip。 | Claude Code |
