...
> ✅ **狀態:SHIPPED v1.0(2026-08-01)— M1–M6 全數完成**
> **交付**|租戶設定 + 個人設定(動態繼承)· 使用者管理(管理員建帳號 + 一次性初始密碼 + 停權)· 帳號安全(裝置清單 / 強制登出 / 認證稽核 / 密碼政策 / 外洩比對)· 通知通道連接(信封加密 + allow-list + 測試發送)· 租戶級事件廣播。實作結果見 §4.1–4.3,FMEA 見 §5,殘留見 §6(**只有 R1 log redact 標為建議先補**)。
> M0 APPROVED v1.0(2026-07-31)— OQ-SC-1..19 全採建議
> **裁定摘要(M2:13=A 管理員建帳號+系統產生初始密碼 · 14=A 15 字 · 15=A 管理員不可自選 · 16=A 單次+短效期+首次強制改 · 17=B 停權即擋登入〔刻意不照 Ragic〕· 18=B 權限轉移列殘留 · 19=A 固定頁面先)**|1=A 納入使用者管理/邀請 · 2=A 預設+個人可覆寫(不做鎖定)· 3=A 動態繼承 · 4=A 編號規則不做 · 5=A 會計期間/營業日曆延 R2 · 6=A 應用層 AES-256-GCM+信封 · 7=A 憑證只回「已設定+更新時間」· 8=A 固定 host 改 allow-list · 9=C 單因子 15 / 已啟用 MFA 者 8 · 10=B 複雜度旋鈕預設關+警語+稽核 · 11=A 做外洩字典比對 · 12=A 認證日誌留 6 個月
...
已交付|裝置清單(含「目前這台」標示)· 強制登出(**連帶撤銷 API 金鑰**,副作用寫在按下去之前)· 認證活動紀錄(保留 6 個月)· 密碼長度政策 OQ-SC-9=C · 外洩/常見/情境密碼比對 OQ-SC-11=A。

**M3 期間查出並修掉四個既有缺口**,四個都是「文件說有、程式碼沒有」或「只在某個環境生效」:

| # | 缺口 | 證據 | 修法 |
|---|---|---|---|
| 1 | 🔴 **登入限流可被 `x-forwarded-for` 繞過** | better-auth 1.6.23 `core/utils/ip.mjs` 逐字:未設 `trustedProxies` 時單一值的 `x-forwarded-for` **照收**;而 `getIp()` 同時是限流的分桶依據。實測輪換假 IP 打 12 次登入,**0 次**被擋 | `mountAuthHandler` 以 Fastify socket peer 覆寫 `x-weyver-peer-ip`(複製完 client headers 之後 set → 偽造不了),`advanced.ipAddress.ipAddressHeaders` 指向它 |
| 2 | 🔴 **初始密碼可永久使用** | M2 承諾「單次使用 + 72 小時效期 + 首次登入強制改」(OQ-SC-16=A),但 `used_at` 從未寫入、登入不查該表 —— ASVS §V6.4.1 逐字禁止初始密碼「become the long term password」 | 登入時認領憑證(過期 → 撤銷剛發的 session 並拒絕);未自設密碼者 **TenantGuard** 對所有 API 回 403;改密碼成功即刪列 |
| 3 | 🔴 **安全機制只在 prod 生效** | 上述閘門原本寫在 AuthGuard,而 dev 走 DevTenantGuard → e2e 實測新同事用初始密碼**直接進到工作區** | 移到 TenantGuard(兩條路的共同出口)。**只在 prod 生效的安全機制等同於從來沒有人驗證過** |
| 4 | 🔴 **暴力防護的主詞錯了** | 63B-4 §3.2.2 要求限制的是 **single account** 的連續失敗;原本只有 per-IP 5 次/分 —— 誤傷共用出口 IP 的整間辦公室(且 better-auth 的限流不分成敗),又擋不住天生分散在大量 IP 的憑證填充 | per-IP 放寬(登入 20/分、全域 2000/分),新增逐帳號連續失敗 10 次鎖 15 分鐘;不做永久鎖定(否則亂打即可把任何人鎖死) |

**誠實標注**|外洩密碼語料(`@zxcvbn-ts/language-common`,MIT,49,233 筆)中**只有 41 筆長度 ≥ 15** —— 在 15 字政策下,語料比對對單因子使用者幾乎空轉,真正的守備由**情境字檢查**承擔(`Weyver-2026-0801!` 長度合格但正是原文點名要擋的)。若只做語料比對就宣稱符合 SHALL,是形式合規、實質空轉。

**殘留**|OQ-SC-10(複雜度 / 定期更換旋鈕,預設關 + 警語 + 稽核)尚未做。⚠️ 該 OQ 自身即標注「稽核標準真的要求定期更換」**未取得一手證據**;在客戶實際提出稽核要求之前先做,等於為一個未確認的需求交付一個我們自己文件裡寫「違反 63B-4」的開關。**建議延到有客戶具體要求時再做**,屆時連同對方的稽核條文一起記錄。

### 4.2 M4 實作結果(2026-08-01)

已交付|信封加密憑證儲存(OQ-SC-6=A)· 只回布林旗標不回值(7=A)· 固定 host 走 allow-list(8=A)· 六通道註冊表 + 測試發送 + 公司層級設定頁。**6 個通道的死控件解除**。

**實測**(dev,整條 HTTP 往返)|非官方網域 → `CHANNEL_URL_NOT_ALLOWED`;回應與 DB 皆無明文(DB 存 `v1.1.…`);測試發送真的打到 Slack 並取回 `HTTP 404 no_team`(假 token 的真實回應)——egress 鏈(DNS 驗證 → pin IP → 禁轉址)全通。

**⚠️ LINE Notify 已於 2025-03-31 終止服務**(官方 closing announcement,2025-04-01 起 API 與 token 簽發全停)。故 LINE 走 Messaging API 的 `api.line.me`,**不是**坊間文件仍在教的 `notify-api.line.me` —— 這一條若照舊資料寫,功能一上線就是死的。
...
## 5. FMEA(M6 完成,2026-08-01)

> **本節逐條回查程式碼**,不採信草案的宣稱。狀態欄的意思是:
> ✅ **已驗證** = 讀過對應程式碼且有測試釘住 · ⚠️ **部分** = 有做但有已知缺口 ·
> ❌ **未做** = 草案寫了但程式碼裡沒有(這正是最危險的一類:看起來已緩解)。

| # | 失效 | 嚴重度 | 狀態 | 實況 |
|---|---|---|---|---|
| C1 | 第三方憑證外洩(回應 / DB / log)| **P0** | ✅ 已驗證(2026-08-01 補完 log 層)| 回應與 DB **已驗證**(整合測試以 JSON 全文比對斷言不含明文;DB 存 `v1.1.…`)。log 層原本**沒有任何 redact**(草案宣稱有,實際沒有),已於 2026-08-01 補上全域 `RedactingLogger`。此前已查明的緩解:SSRF 錯誤只回主機名;`send()` 的失敗訊息不含 secret。現在再加一道:**Telegram 的 token 位於 URL 路徑**這個最危險的形狀,即使未來有人把整個 URL 寫進 log 也會被遮掉 |
| C2 | 測試發送成為打內網 / 打第三方的跳板 | **P0** | ✅ 已驗證 | allow-list(整個 host 比對,擋後綴偽裝)+ 儲存當下即擋 + `https.request` 不跟隨轉址 + DNS 解析後 pin IP。**M6 補上專屬限流 10 次/分** —— 原本吃全域 300/分,等於一台每分鐘 300 發的放大器 |
| C3 | 邀請被跨租戶濫用 / 冒領 | **P0** | N/A | OQ-SC-13=A 選了「管理員建帳號 + 一次性初始密碼」,**邀請信路徑未實作** → 此失效目前不存在。B 路徑動工時本條復活(且 `requireEmailVerificationOnInvitation` 已預先設為 true) |
| C4 | 強制登出後 session 未真正失效 | **P0** | ✅ 已驗證 | 刪 session **且撤 API 金鑰**;整合測試斷言剩下的只有「目前這台」、金鑰全數 revoked。UI 在按下去**之前**講明副作用 |
| C5 | 改租戶時區導致既有單號日期段跳動 | P1 | ❌ 未做 | 目前改時區**無警告、無稽核**。既有記錄不回溯(單號在建立時就固化)故不會竄改歷史,但下一張單的日期段會跳 → 見 §6 殘留 R2 |
| C6 | 設定頁把個人值寫成租戶值(或反之)| P1 | ✅ 已驗證 | 兩者分表(`tenants` / `user_pref`)分端點;`actorId` 一律取自 `TenantContext` 不接受呼叫端指定;`settings-center.spec` 覆蓋 |
| C7 | 密碼政策旋鈕被開成違反 63B-4 而無人知情 | P1 | N/A | OQ-SC-10 建議延後(該 OQ 自身標注「稽核標準要求定期更換」未取得一手證據)→ 旋鈕不存在,失效不存在 |
| C8 | 裝置清單顯示的地點被當成事實 | P2 | ✅ 已驗證 | **刻意不顯示地點**(Microsoft 自己免責「may look like you signed in from somewhere that's not your actual location」);只顯示 IP / 裝置 / 瀏覽器 / 最後活動 |

### 5.1 M3–M5 新增的失效模式

| # | 失效 | 嚴重度 | 狀態 | 緩解 |
|---|---|---|---|---|
| C9 | 🔴 **帳號鎖定被拿來當 DoS 工具** —— 知道某人 email 即可持續讓他登不進來 | **P0** | ✅ 已修(M6)| OWASP **明文點名**此風險(「care must be taken to prevent it from being used to cause a denial of service by locking out other users' accounts」),原本的固定 15 分鐘正是它警告的形狀。改為它給的替代做法:**指數遞增**(第 10 次失敗鎖 1 秒、之後倍增、上限 15 分鐘)。⚠️ **殘留**:持續攻擊仍會把帳號壓在上限;原文給的逃生口是「forgotten password functionality」,**本專案尚無該流程** → §6 殘留 R3 |
| C10 | KEK 遺失 → **全租戶的第三方憑證永久解不開** | **P0** | ⚠️ 部分 | prod 未設即 fail-fast(不會靜默用佔位金鑰);信封設計讓輪替不必碰明文,`rewrapSecret` 已實作並測試。**但無輪替 CLI、無備援程序文件** → §6 殘留 R4。失效後果可控:憑證重填即可,**不會遺失業務資料** |
| C11 | 廣播風暴 —— 大量記錄變更把公司頻道洗版 / 撞對方限流 | P1 | ❌ 未做 | 廣播**不做去抖動合併**(刻意:群組看的是事件流)。批次匯入 1000 筆會產生 1000 則廣播。目前唯一的閘門是管理者自己不勾 `record.updated` → §6 殘留 R5 |
| C12 | 廣播內容把欄位值推給無權限的人 | **P0** | ✅ 已驗證 | 標題由 `safeTitle` 產生(表單名 + 記錄編號),整合測試斷言形狀為 `^.+ #\d+$`;UI 在勾選框旁明講此事 |
| C13 | 初始密碼閘門把使用者鎖死在門外 | **P0** | ✅ 已驗證 | 閘門在 `TenantGuard`,而 `/api/auth/*` 是 Fastify 原生路由**不經該 Guard** → 改密碼這條路恆通。`onboarding.spec` 走完整迴圈驗證 |
| C14 | 安全機制只在 prod 生效 | **P0** | ✅ 已修 | 初始密碼閘門原本只寫在 AuthGuard,dev 走 DevTenantGuard 完全不執行(e2e 實測新人直接進工作區)。已移到兩條路的共同出口 |
| C15 | 憑證變更無稽核 —— 事後查不出誰換的 | P1 | ⚠️ 部分 | 有 `updated_by_actor_id` + `secret_fingerprint`(可判斷「值換了」)但**不寫入稽核表** → §6 殘留 R6 |

### 5.2 P0 結論

**8 條 P0 中:6 條已驗證緩解、2 條 N/A(功能未實作故失效不存在)。**
C1 於 2026-08-01 補上全域 log 遮蔽後結案 —— 原本的缺口不是「現在會外洩」,
而是「沒有任何機制阻止未來新增外洩路徑」。


---

## 6. 殘留(M6 盤點,依上 prod 的阻擋程度排序)

| # | 殘留 | 阻擋上 prod? | 說明 |
|---|---|---|---|
| ~~**R1**~~ ✅ | ~~log redact 規則~~ | **已完成 2026-08-01** | 掛全域 `RedactingLogger`(NestJS 自訂 logger,**不換整套日誌堆疊**)。兩種遮法:**鍵名**(password / token / secret / authorization…)+ **值的形狀**(Telegram 的 `/bot<token>/`、我方信封密文 `v1.…`、Slack/Teams/Discord webhook 路徑、`Bearer …`、連線字串密碼)。鍵名遮不到「有人把整個 URL 串進訊息裡」,故兩者都要。連線字串**只遮密碼保留 host** —— 全遮會讓「連哪一台」也查不出來,而那正是查問題要的。11 條測試,含循環參照不當機、一般訊息不被過度遮蔽 |
| **R2** | 改租戶時區的警告與稽核 | 否 | 既有記錄不回溯(單號建立時固化),故不竄改歷史;但下一張單的日期段會跳,使用者應被告知。屬 C5 |
| **R3** | 忘記密碼流程 | 否(但 C9 殘留因它成立)| OWASP 對「鎖定變成 DoS」給的逃生口正是它。同時也是 M2 選 A 路徑(管理員發初始密碼)之後,使用者自助救援的唯一途徑 |
| **R4** | KEK 輪替 CLI + 備援程序 | 否 | `rewrapSecret` 已實作並測試,缺的是操作入口與「KEK 存哪、怎麼備份」的文件。**失效後果可控**:憑證重填即可,不會遺失業務資料 |
| **R5** | 廣播風暴防護 | 否 | 批次匯入 1000 筆會產生 1000 則廣播。最小可行緩解是**逐租戶逐通道的每分鐘上限 + 超過即合併成一則「N 筆變更」**;在有客戶真的接上通道之前不急 |
| **R6** | 憑證變更寫入稽核表 | 否 | 目前有 `updated_by_actor_id` + 指紋,查得出「誰在何時換過、值不同了」,但不在統一的稽核流裡 |
| **R7** | SMTP 的測試發送 | 否 | 非 HTTPS,需走 `EmailChannel`;目前該通道可設定但按不了「測試發送」(UI 已據 `channel !== "smtp"` 隱藏按鈕,不是死控件) |
| **R8** | 邀請信路徑(OQ-SC-13 的 B)| 否 | 卡在 email 驗證流程;A 路徑已讓「第二個員工進不來」這件事解決 |
| **R9** | 資料管理者權限轉移(OQ-SC-18=B)| 否 | 與 #104(簽核代理人 / 動態主管解析)同一批問題 |
| **R10** | 使用者做成「一張表單」(OQ-SC-19=B)| 否 | 很符合定位且值得做,但屬引擎能力的延伸,不是設定中心的一部分 |

**上 prod 的硬前提**|~~R1~~ **已完成**;其餘皆為加法,不阻擋。
