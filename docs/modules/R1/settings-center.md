...
| # | 失效 | 嚴重度 | 狀態 | 實況 |
|---|---|---|---|---|
| C1 | 第三方憑證外洩(回應 / DB / log)| **P0** | ⚠️ 部分 | 回應與 DB **已驗證**(整合測試以 JSON 全文比對斷言不含明文;DB 存 `v1.1.…`)。**但草案宣稱的「log redact 規則」不存在** —— 專案目前沒有任何 redact 設定(AGENTS 要求的 nestjs-pino 亦未接)。已查明的緩解:SSRF 錯誤只回主機名不回完整 URL;`send()` 的失敗訊息不含 secret。**殘留**:Telegram 的 token 位於 URL 路徑,任何未來新增的「把 URL 寫進 log」都會外洩 → 見 §6 殘留 R1 |
...
**8 條 P0 中:5 條已驗證緩解、2 條 N/A(功能未實作故失效不存在)、1 條(C1)為部分。**
C1 的缺口是「log redact 規則不存在」——**目前沒有任何路徑會把憑證寫進 log**(已逐條查過),
但也**沒有任何機制阻止未來新增這種路徑**。上 prod 前應補 §6 R1。
...
| # | 殘留 | 阻擋上 prod? | 說明 |
|---|---|---|---|
| **R1** | log redact 規則 | ⚠️ **建議先補** | 目前**沒有任何路徑**把憑證寫進 log(已逐條查證),但也沒有機制阻止未來新增。AGENTS 要求的 nestjs-pino 尚未接入 —— 接的時候一併設 `redact`,把 `secret` / `token` / `authorization` / `password` 列進去。成本小、防的是「以後某人順手 log 一個物件」 |
...
**上 prod 的硬前提**|R1 建議先補;其餘皆為加法,不阻擋。
