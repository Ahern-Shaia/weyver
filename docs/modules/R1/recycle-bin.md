...
| R5 | 還原寬表超時 | ✅ **2026-08-03 對碼查證後判定不適用,結案不做**。還原總共只有 **3 句 UPDATE**(`form_def` 1 列 · `field_def` 一句 `inArray` bulk · `trash_entry` 1 列),**語句數與欄位數無關,且完全沒有 DDL** —— 軟刪從來沒有拆掉物理結構(欄與表要到 purge 才 `DROP`)。Baserow #5101 會超時是因為**它的還原重建 schema**,那個前提在本架構不成立。背景 job 是為不存在的問題加複雜度 | P1 | ✅ |
| R6 | 7 個 Tier-1 `deleted_at` 實體漏納 TTL → 合規破口留在角落 | purge **直接掃各表 `deleted_at`** 而非掃 entry —— 沒有 entry 的軟刪資料照樣清,結構上不會有死角 | P1 | ✅ |
| **R7** | 🔴 **attnum 耗盡撞 1600 欄硬牆** —— 既有的 `maxFieldsPerForm` 配額只數**活著的**欄位,所以一張反覆加欄刪欄的表**可以永遠通過配額**,卻在某天撞上 PG 硬牆,而使用者看到的是「tables can have at most 1600 columns」這種對他毫無意義的訊息 | ✅ **2026-08-03**。`addField` 於 advisory lock **之後**量 `pg_attribute` 的 `max(attnum)`(真正的高水位;`field_def` 列數不是)—— 1400 拒絕並給出路(訊息明說「刪欄不會回收額度,需複製到新表單重建」)、800 起把用量寫進 `ddl_audit` 並記 warn。整合測以單句 ALTER TABLE 墊高水位,並**反向驗證「刪掉 200 欄仍然被擋」**。⚠️ 無外部告警接收端(GlitchTip 未接)→ 日誌與 `ddl_audit` 即告警面;**自動重建表的維護作業未做** | P1 | ✅ |
| **R8** | 🔴 **硬刪後檔案實體殘留** —— 記錄真的被 DELETE 了,`file_object` 卻還是 `bound`,而孤兒回收器只撿 `orphaned`:**DB 裡查不到、儲存體上還在、額度持續被佔**。對合規而言等於沒刪 | ✅ **2026-08-03**。purge 記錄 / 欄位 / 表單三條路徑各自把對應 `file_object` 標成 `orphaned`,交棒給既有回收器。**只標狀態、不在此刪位元組** —— 物件刪除不是交易性的,放進 tx 裡一旦回滾就是「資料還在、檔案沒了」;交棒同時拿到回收器已寫好的觀察期、縮圖連帶刪、單檔失敗不中斷整批 | P1 | ✅ |
...
| 日期 | 版本 | 變更 | 作者 |
|---|---|---|---|
| 2026-08-03 | v1.1 | **三條 P1 殘留全數結案(R5 / R7 / R8),模組無殘留**。**R8 附件連帶回收**:purge 記錄 / 欄位 / 表單時把對應 `file_object` 標 `orphaned` 交棒既有回收器 —— 在此之前檔案是「DB 查不到、儲存體還在、額度持續被佔」,合規上等於沒刪;刻意只標狀態不在 tx 內刪位元組(物件刪除非交易性,回滾就變成「資料還在、檔案沒了」)。**R7 attnum 一生額度**:本機實測 30 次 add/drop 後 `VACUUM FULL`,`max(attnum)` 仍是 31 —— 確認 PG 核心「never recycle attnums」;既有配額只數活著的欄位,故反覆加刪的表**可以永遠通過配額卻某天撞牆**。`addField` 改為在 advisory lock 後量 `pg_attribute` 高水位,1400 拒絕(訊息講出路)、800 起寫進 `ddl_audit`;測試反向驗證「刪掉 200 欄仍被擋」。**R5 還原背景化:查證後判定不適用,結案不做** —— 還原只有 3 句 UPDATE 且與欄位數無關、無 DDL,因為軟刪從未拆掉物理結構;Baserow #5101 超時的前提是「還原會重建 schema」,本架構不成立。**殘留**:自動重建表的維護作業未做 / 無外部告警接收端(GlitchTip 未接)。api 32 綠(ddl + trash) | Claude Code |
