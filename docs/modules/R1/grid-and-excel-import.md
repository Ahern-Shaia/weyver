...
> **與現行實作的差距(2026-08-04 對碼更正,`_audit/giants-shoulders-audit-C.md` §3-2)**
>
> ⚠️ **這份清單原本兩邊都錯** —— 有兩項早就做完了卻仍寫著沒做,而真正缺的沒有人在追。
> 這正是 `pitfall_stale_todo_already_done`:**待辦不會自己過期,下一個讀到的人會當成現況。**
>
> | 項目 | 原記載 | **對碼結果(2026-08-04)** |
> |---|---|---|
> | 分層取樣 | 「取樣仍為前 50 列」 | ✅ **已做**:`excel/import.ts:6-23` 頭 50 / 中 100 / 尾 50,附註解說明理由 |
> | `dense` 模式 | §「大檔」列寫「無 `dense`」 | ✅ **已做**:`excel/parse.ts:29,58` 皆 `dense: true` |
> | 命中門檻 | 100%(`allMatch`) | 🔴 **仍是** `import.ts:38` `allMatch`,未改 ≥95% |
> | 日期推斷 | 走文字 regex | 🔴 **仍是**,未用 `cellDates:true` 取 Excel 序列值 |
> | singleSelect 門檻 | `min(10, rowCount × 0.3)` | 🔴 **仍是**,小樣本易誤判 |
> | 逃生鍵 | 無「全部設為文字」 | 🔴 **仍缺**(`panel.tsx`) |
> | Web Worker / 超大檔後端路徑 | 無 | 🔴 **仍缺** —— 「客戶 3 萬列舊 Excel」的硬傷未解 |

### ✅ 產品方向確認

**Ragic 建表時是引導使用者逐欄指定型別、不靜默猜**;Airtable 的自動推斷可關閉。
→ 本專案的「**猜 + 預覽可覆寫**」其實比 Ragic 前進,方向對,值得保留為差異化;
要補的只有推斷規則的三個經典坑(**門檻 / `cellDates` / singleSelect 門檻,三個都還在**)與逃生鍵。
