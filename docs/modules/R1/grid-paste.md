...
## 9. 順帶修掉的「靜默少做」:Excel 匯出只給已載入的頁(2026-08-06)

`docs/25` B 段那一列裡夾著一句「Excel export 僅含已載入筆數」——
查證後仍然成立,而且它的形狀是這個 repo 反覆踩的那一種:
**檔案打得開、格式也對,只是資料少了,而使用者不會發現。**

改成匯出前先把剩下的頁抓完;超過瀏覽器上限(10,000)時**明說**,
並指向設定中心的租戶級匯出(R1·I-1,非同步產檔、不受此限)。

### 9.1 🔴 閉包過期:非同步迴圈裡讀元件狀態

第一版寫成:

```ts
while (recordsQuery.hasNextPage && records.length < CAP) {
  await recordsQuery.fetchNextPage()
}
```

`records` 與 `hasNextPage` 都是 **render 當下捕捉的閉包** —— 在 `await` 之後
它們永遠是舊值,於是迴圈只跑一次就以為抓完了,匯出的仍是第一頁。
**型別檢查完全不會抱怨。** 改成一律用 `fetchNextPage()` 的**回傳值**。

e2e 抓到(建 250 筆、匯出只有 200)。

### 9.2 兩個順帶

- **畫面上那句話變成假的了**:狀態列原本寫「(匯出僅含已載入 N 筆)」——
  功能改了而說明沒改,比一開始就沒寫更糟,使用者會照著它做錯的決定。已移除。
- **分頁大小要去讀實際用的那一支**:測試第一版照著另一支 hook 的 `limit=50`
  建了 70 筆,而 `useInfiniteRecordsQuery` 的 `pageSize` 預設是 **200** ——
  只有一頁,「載更多」不出現,**對照組直接紅**。
