...
| # | 失效 | 嚴重度 | 緩解 |
|---|---|---|---|
| D1 | A1 合併後,某個原本正確的畫面變錯(如 member 名被吃掉) | P0 | 四個呼叫點逐一實走;單元測涵蓋 member / 附件 / SOURCE_MARKERS 三個獨有分支 |
| D2 | 自製輸入吃掉了原生控件的行動裝置體驗(iOS 滾輪選擇器) | P1 | 窄螢幕保留原生控件 fallback,並在 §0.3-bis 標明:**行動端未量測** |
| D3 | 寬鬆解析把 `0305` 解成錯的年份(跨年當下) | P1 | 解析後**回填成完整格式顯示**,使用者看得到系統補了什麼(Ragic 同樣是回填) |
| D4 | `dateFormat` 只做在前端,匯出 / 列印 / webhook 各自為政 | P1 | A1 的單一來源就是為了這個;匯出與列印的呼叫點列入驗收 |
...
| **M3** | A3 自製日期輸入 | 寬鬆解析五列 + 鍵盤日曆 + 失敗不清空 |
| **M4** | FMEA + docs 回填 | §9 P0 全緩解;`docs/25` 不灌水 |
