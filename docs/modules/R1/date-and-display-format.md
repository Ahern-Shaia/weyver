...
| **D8** | 匯出 Excel 是 **client-side**,走的是自己的一套值轉換 | A1 只掃了「顯示」的呼叫點,沒掃匯出 | ✅ **2026-08-04 查完:不是缺陷,是兩種刻意不同的匯出**。⚠️ 而我當時寫「A1 沒掃匯出」**是錯的** ——列表頁的匯出就在 `collection-view.tsx` 裡,M1 早就一併接上了 `fmtCtx`。**又一次憑假設寫待辦**。詳見 §9.2 |

---

### 9.2 D8 的結論:兩種匯出,兩種職責(2026-08-04)

| 匯出 | 職責 | 值轉換 | 現況 |
|---|---|---|---|
| 列表頁「匯出 Excel」 | **所見即所得**(看的便利) | `formatFieldValue(..., fmtCtx)` → 金額 `128,400.00`、日期依欄位格式 | ✅ M1 已接上,**e2e 現在讀 xlsx 內容逐格驗** |
| 設定中心的租戶封存(CSV + zip) | **帶得走**(供再匯入) | `csv.ts` 的 `toText` → 原值 `128400.0000`、ISO 日期 | ✅ 刻意如此,已補單元測釘住 |

**兩者若哪天被「統一」,失效方式完全不同**:
前者少一列沒人會死;**後者把 `128400.0000` 變成 `128,400.00`,再匯入時就是壞資料**
(千分位逗號在 CSV 裡還會多切一欄)。故兩側各自釘了測試。

⚠️ **原本這條 e2e 只驗副檔名** —— 連「檔案裡有沒有東西」都沒問。
M1 把顯示收成單一來源之後,那是**唯一**能證明匯出真的跟著畫面走的地方,已改為讀 xlsx 逐格斷言。

⚠️ **我自己犯的**:D8 原文寫「A1 只掃了顯示的呼叫點,沒掃匯出」——
而列表頁的匯出就在 `collection-view.tsx` 裡,是 M1 掃過的七個呼叫點之一。
**憑假設寫下的待辦,和憑假設寫下的結論一樣危險** —— 它會讓下一個人去修一個不存在的問題。
