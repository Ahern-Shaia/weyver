...
### 0.5 建議重裁 / 新增 — ✅ **已裁定 2026-08-03**

> **裁定摘要**|R1 標 SUPERSEDED 並把兩項未涵蓋者移交 wysiwyg · **R2 選 (b) 移除**(已執行)·
> R3 歸 `view_def` 走 Glide `onColumnResize` · **R4 / R5 見下方各自裁定** ·
> **R6 已修**(`readonly` / `help` / `placeholder` 於填單端生效,commit `2460d3d`;
> `statics` / `sections` 隨 R1 移交)· R7 獨立模組評估。
>
> | # | 裁定 | 說明 |
> |---|---|---|
> | R1 | **標 SUPERSEDED,兩項未涵蓋者移交 `form-designer-wysiwyg`** | (a) 分段收合 → 子表 lazy load 是**載入效能**不是視覺偏好,不得隨分段模型一起被吃掉;(b) 「單一子表格不可拆段」須補進 schema 約束 |
> | R2 | ✅ **(b) 移除 schema 兩個 key**(已執行) | reader 0 / writer 0 的欄位**不是待辦,是陷阱** —— 下一個人看到 schema 會以為做過了(已發生一次)。真正有人要的那個功能歸 R3。<br>🔴 **後續發現(2026-08-03)**:`form-designer-wysiwyg` **OQ-FDW-7 的依據②正是這兩個 key**(「我們自己的 design doc 與 schema 早就這樣規定」)。兩份都已裁定卻互相依賴,**而沒有任何機制在其中一份改變時提醒另一份**。已收斂:OQ-FDW-7=A 維持(依據①Ragic 逐字、③遷移忠實度獨立成立),兩個 key 由該模組**在落地試算表模型時同批帶 writer 重新引入** |
> | R3 | **列表頁欄寬歸 `view_def`**,沿用 Glide 內建 `onColumnResize`(全 repo 亦零使用)| 表單畫布的欄寬與列表頁的欄寬**是兩件事,不共用資料**。表單畫布的寬度已由 `colSpan` 表達。落地時對齊 Glide 預設 min/max(50/500)|
> | R4 | ⚠️ **不現在裁,但綁一個觸發條件** | 「12 欄夠不夠承接 Ragic 21 欄」**無法憑推理回答**,要看真實遷移表單。**觸發**:`form-templates` 或首個 pilot 遷移取得真實表單後即裁,不得再延。現階段 `cols` 保留可設(schema 已允許 1–50),只是無 writer |
> | R5 | ✅ **以實作為準:1 欄位 = 1 格**,格內 `112px 1fr` 子格線 | 目前兩份文件與實作**三方不一致**,而遷移換算會直接踩到。既然實作已出貨且 e2e 綠,**改文件不改碼**;§4.1 之「header 於 col、value 於 col+1」為誤述,已標 |
> | R6 | ✅ **已修**(`readonly` / `help` / `placeholder`)| 其中 `readonly` 是正確性問題不是缺功能 —— 設計器勾了唯讀、填單照樣能改。並已誠實標示為「排版層,非權限」 |
> | R7 | **移交獨立模組評估**,不在本模組續攤 | `styleSchema` 5 屬性零 reader 無 UI;Ragic 有框線 / 複製樣式 / 主題三層,是一個完整題目不是殘留 |
