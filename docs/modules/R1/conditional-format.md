...
> ✅ **狀態:SHIPPED v1.3(2026-08-04)** —— **C-2 + C-3 全數落地**。
> C-2:color / hide / readonly / 分段 / 訊息(§11)· C-3:必填 / 動作按鈕 / 開始簽核,**伺服器強制**(§11-bis)。
> 歷程:v1.0 2026-07-28(M1–M4 + FMEA G1–G7)。
...
## 11. C-2 續章:分段 + 顯示訊息(2026-08-04)

> C-2 的前半(`hide` / `readonly`)已於 `db026ba` / `4ae56e8` 出貨。本節處理後半:
> **顯示、隱藏或上鎖分段** 與 **顯示訊息**。三站逐站查過,查證日 2026-08-04。

### 11.1 站③|Ragic 官方逐字(`doc/6`,本機鏡像)

**分段**|
> 「當表單有 **表單分段** 時,可以設定指定條件達成時,要顯示、隱藏或上鎖分段。
> 就可以將相關欄位設成同一分段後,再透過條件式格式一次設定,**就無需逐一針對各欄位進行設定**。
> 若選擇將分段 **上鎖** ,該分段會變為 **唯讀** 狀態。
> 注意:分段內的 **動作按鈕不會上鎖** ,需要另外設定條件式格式將動作按鈕上鎖。」

🔴 **這段話直接決定了資料形狀**:分段效果是**批次捷徑**,不是新的效果種類 ——
它的效果就是欄位級的那一組(顯示 / 隱藏 / 唯讀),只是一次套用到一群欄位。

**顯示訊息**|
> 「可以設定指定條件達成時顯示訊息。可以在訊息中搭配 **欄位參數** :
> `{{fieldName_欄位編號}}` 及 `{{fieldValue_欄位編號}}`。……
> 欄位參數**不支援子表格欄位**,僅支援獨立欄位。」

### 11.2 站①|自家 repo(對碼複驗)

| 觀察 | 含意 |
|---|---|
| `sectionSchema` 以 `fromRow`/`toRow` 界定成員,**設計器有建立 / 刪除 UI** | 分段成員關係已可推導,不必新增資料 |
| 🔴 `fieldLayoutSchema.sectionId` **零 reader 零 writer** | 與 `a66f110` 移掉的 `colWidths` / `rowHeights` **同型**。留著會變成第二套成員關係 → **本批移除** |
| 🔴 **填單與記錄頁完全沒有渲染 `layout.sections`** —— 只有設計器畫布看得到 | 「隱藏分段」在填單畫面的可見結果就是**其欄位一起消失**;分段標頭本來就沒畫。誠實列殘留,不假裝有 |
| `evaluateFieldStates` 的仲裁單位是**欄位名稱**,後者覆蓋 | 分段若另立一軸,跨軸順序無從定義 —— 正是 §10-bis 選項 A 被否決的「兩份真相」 |
| 2026-08-04 剛修過**公式的隱藏欄洩漏**(污染閉包) | `{{fieldValue}}` 是**同一個形狀的旁路**:讓看不到的值從另一個出口流出來 |

### 11.3 裁定(OQ-CF-9 ~ 11)

| # | 議題 | 裁定 | 依據 |
|---|---|---|---|
| **OQ-CF-9** | 分段是新效果類還是目標選擇器 | **目標選擇器** —— 規則加 `targetSections[]`,求值時展開成該區間內的欄位,套**同一組** effects | 官方逐字「無需逐一針對各欄位進行設定」= 批次捷徑;且維持**單一仲裁空間**(每欄位),後者覆蓋跨分段級 / 欄位級自動一致 |
| **OQ-CF-10** | 「上鎖」要不要新效果 | **不要**,`上鎖 = readonly` | 逐字「若選擇將分段上鎖,該分段會變為**唯讀**狀態」 |
| **OQ-CF-11** | 訊息的欄位參數用欄位 id 還是欄名 | **欄名**,語法 `{{fieldValue:欄名}}` / `{{fieldName:欄名}}` | 我方 `conditions.field` 與 `targets` **一律用欄名**,再引進一套 id 指涉等於要使用者學兩套。分隔符改冒號並**只切第一個**:欄名可以含底線,Ragic 用 id 才沒有這個歧義 |

**🔴 訊息的兩條硬約束**|

1. **插值不得成為遮罩的旁路。** 求值在前端,`values` 已過 `maskRead` ——
   對此人隱藏的欄位**根本不在 values 裡**。此時**不得留空**(留空會讓人以為那欄沒資料),
   回具名的 `(無權檢視)`。沿用 `pivot-and-charts` 與 `link-options` 的同一裁定:**寧可具名,不要靜默。**
   ⚠️ 條件式**隱藏**的欄位不在此列 —— 那只是版面層,官方亦逐字說明
   「只會作用於排版介面上,於修改資料紀錄或通知信中仍會顯示該欄位的資料」。
2. **訊息是純文字,不吃 markdown / HTML。** 文字由使用者自訂、值由資料帶入,
   兩者都是不可信輸入。React 預設跳脫即足夠,**條件是永遠不要走 `dangerouslySetInnerHTML`** —— 加測試釘住。

**範圍**|訊息只在**記錄頁 / 填單頁**。列表頁一列一則訊息沒有意義,設計器在列表頁分頁不提供,求值端亦略過。

### 11.4 落地(2026-08-04)|✅ **C-2 全數 SHIPPED**

後端 `layout-specs` 加 `message` 效果與 `targetSections`、**移除死欄位 `sectionId`**;
前端 `sectionMembers` / `renderMessage` / `evaluateMessages`;
設計器新增分段多選與訊息輸入(**自 411 行的 `conditional-format.tsx` 拆出 `RuleEffects`**);
`RuleMessages` 同時掛在填單面板與記錄頁。
api 1063 · web 259 · e2e 145 全綠;Playwright MCP 實走設計器 → 存版面 → 讀 API 確認持久化 → 填單看到插值訊息。

**§5-bis 補 FMEA**|

| # | 失效 | 嚴重度 | 緩解 |
|---|---|---|---|
| G8 | `{{fieldValue}}` 成為**欄位遮罩的旁路** —— 看不到的欄位從訊息流出來 | **P0** | 求值吃的是已過 `maskRead` 的 `values`;鍵不在就回具名的「(無權檢視)」。⚠️ 這與 2026-08-04 修的公式污染閉包**是同一個形狀**,故從第一版就走可見值而非事後補 |
| G9 | 訊息文字 / 帶入值被當標記解析 → XSS | **P0** | 渲染集中在 `RuleMessages` 一個元件、純文字;e2e 打 `<b>PO-9</b>` 進去並斷言 `b` 元素數為 0 —— 若日後有人為了「支援粗體」接上 `dangerouslySetInnerHTML`,那條會紅 |
| G10 | 分段展開把條件欄也掃進去 → 使用者連輸入的欄位都不見 | P1 | 分段展開後與 `targets` **併集**,只有兩者皆空才退回條件欄;e2e 斷言段外的單號仍在 |
| G11 | 分段被刪除後規則指向孤兒 id | P1 | 查無該分段 → 略過該分段、其餘照套(同目標欄已刪的處置);單元測固化 |
| G12 | 設計器允許把 `fromRow`/`toRow` 拖成反向 → 靜默變成空分段 | P2 | `sectionMembers` 取 min/max;單元測固化 |

**不在本批(誠實列殘留)**|
- **「顯示」效果**(預設隱藏、命中才出現)—— 需要反向的預設狀態推導,獨立一項。
- **分段標頭在填單畫面的視覺呈現** —— 目前只有設計器畫得出分段。
- **「分段內動作按鈕不會上鎖」** —— 動作按鈕屬 C-3;但**UI 現在就要講明**,否則使用者以為鎖住了。

---

## 11-bis. C-3|伺服器強制(2026-08-04)—— ✅ SHIPPED

> §10-bis 逐字說「原 OQ-CF-8 理由 (c) 風險與工期跳級對這個選項仍然成立」,
> 指的正是這三項:**必填 / 動作按鈕 / 開始簽核**。它們的共同點是
> **只在前端做就等於沒做** —— 打 API 就繞過去了。

### 11-bis.1 站③|官方逐字(`doc/6`,查證 2026-08-04)

> 「條件式格式也能夠在指定條件達成時,將欄位設定為**必填**。
> 如果一個欄位已套用**欄位必填設定**時,設定條件式格式時則**無法選擇該必填欄位**。」
>
> 「你也可以透過條件式格式來**顯示、隱藏或上鎖動作按鈕**(包含合併列印按鈕和客製列印報表按鈕)……
> 如果是要**上鎖**動作按鈕的話,還可以**客製提醒訊息**。」
>
> 「你可以透過條件式格式來設定簽核規則決定是否**顯示或隱藏「開始簽核」按鈕**。
> 例如:當「銷售訂單」的總金額超過一萬元時才需要進行簽核流程。」

### 11-bis.2 站①|自家 repo 給了答案

**問題**|伺服器要執法,就得會求值;而求值器在前端。照抄一份到後端,
兩份會漂移 —— 而漂移的症狀不是樣式不一致,是**「畫面說可以存、伺服器說不行」**,
使用者完全看不出自己錯在哪。

**答案本來就在 repo 裡**|`@weyver/formula` 是前後端共用的套件,
`formula-preview.ts` 的檔頭逐字寫著「與後端 `FormulaService.computeRecord` 用
**同一套 `@weyver/formula` 引擎**」。照辦:求值器移進 **`@weyver/rules`**,
`apps/web/src/lib/engine/conditional-format.ts` 降為 re-export。

⚠️ **站② 的教訓當場又踩了一次**:`packages/rules` 的內部 import 寫成 `./evaluate.js`
(NodeNext 慣例),而既有的 `packages/formula` 用的是**無副檔名**——
Next 的 webpack 解不到,整個 `/app` 500。**先看隔壁那個 package 怎麼寫**,
不要照通則寫。

### 11-bis.3 裁定(OQ-CF-12 ~ 14)

| # | 議題 | 裁定 | 依據 |
|---|---|---|---|
| **OQ-CF-12** | 必填的仲裁:覆蓋還是聯集 | **聯集** —— 靜態必填不會被規則拿掉 | 官方對**唯讀**才說「條件式格式必會優先於欄位屬性設定」;對必填說的是「已套用必填設定時,設定條件式格式**無法選擇**該欄位」= **設計期排除**,不是執行期覆蓋 |
| **OQ-CF-13** | 按鈕 / 簽核要不要新的效果類 | **不要** —— 它們是**目標軸**(`targetButtons` / `targetApproval`),效果沿用 `hide` / `readonly` | 官方對按鈕、分段、欄位用的是**同一組動詞**(顯示 / 隱藏 / 上鎖)。另立 `buttonHide` / `buttonLock` 等於同一個心智動作有兩套名字 —— 同 OQ-CF-9 的理由 |
| **OQ-CF-14** | 「客製提醒訊息」放哪 | **同一條規則上的 `message` 效果** | 不必新欄位;而且訊息本來就支援欄位插值,理由可以寫得具體 |

**🔴 三條硬約束**|

1. **部分更新要先合併再求值。** `PATCH` 只送改動的欄位,而規則的條件可能引用
   這次沒送的欄位 —— 只拿 patch 求值,條件會憑空不成立,**必填就靜靜地消失**。
   故 `updateOne` 先在同一交易內讀回該列再合併。
2. **被規則隱藏 → 連靜態必填一起放掉。** 官方逐字「當欄位因條件式格式被隱藏時,
   系統會略過檢查必填及輸入檢查」。不放掉的話,使用者要填一個看不見的欄位,
   而畫面上完全無從得知為什麼存不了。
3. **閘門用未遮罩的值求值。** `evaluateActionGate` 刻意不走
   `getRecord(..., permissions)` —— 閘門問的是「**這筆記錄的狀態**允不允許做這件事」,
   不是「這個人看得到什麼」。用遮罩後的值求值,一個看不到「金額」的人,
   他的條件自動不成立,「金額超過一萬才需要簽核」對他就失效了。

### 11-bis.4 落地

後端 `RecordService.conditionalAttrs`(必填 / 略過檢查)+ `evaluateActionGate`;
`ButtonService.execute` 與 `ApprovalService.submit` 各自擋下並回設計者寫的理由;
前端填單星號改吃 `resolveFieldAttrs`,記錄頁按鈕與送簽鈕隱藏 / 停用並顯示理由;
設計器加「設為必填」與動作目標。

`@weyver/rules` 50 單元測 · api 1072 · web 216 · e2e 147 全綠。

### 11-bis.5 FMEA

| # | 失效 | 嚴重度 | 緩解 |
|---|---|---|---|
| G13 | 只在前端擋必填 → 打 API 就繞過 | **P0** | 伺服器同一份求值器再判一次;整合測試**完全不經過畫面**直接打 API 斷言 422 |
| G14 | 部分更新讓條件憑空不成立 → 必填消失 | **P0** | 同交易內讀回該列再合併求值;整合測試「條件欄不在 payload 裡,必填仍成立」 |
| G15 | 前後端求值漂移 → 畫面說可以存、伺服器說不行 | **P0** | 求值器單一來源 `@weyver/rules`;e2e 同時斷言畫面的星號與伺服器的 422 |
| G16 | 閘門用遮罩值求值 → 看不到條件欄的人自動豁免 | **P0** | `evaluateActionGate` 走未遮罩讀取,並在程式碼註解寫明理由 |
| G17 | 按不動又不說原因的按鈕 | P1 | 沒設客製文案時給具名預設(「此記錄目前的狀態不允許執行這個動作」);前端 `title`、後端錯誤信封都帶 |
| G18 | 規則把使用者鎖死:必填一個他填不了的唯讀欄 | P1 | 唯讀欄不標星號(既有);隱藏欄放掉必填。**殘留**:規則同時把某欄設為 `readonly` + `required` 時,伺服器仍會要求填 —— 設計器未擋,列殘留 |
...
| G2 | 使用者以為「條件式隱藏」可保護敏感欄 | ⚠️ **2026-08-04 前提已不成立** —— 原緩解是「本模組**不提供**條件式隱藏(OQ-CF-8=A)」,而 C-2 已出貨 `hide`。**這個 ✅ 建立在「沒做這個功能」之上,而功能已經有了。**現行緩解在 §11.4 G8 與設計器的常駐警語(「隱藏與唯讀為版面層效果,擋不住 API」),不在此列 | P0 | 🔁 見 §11.4 G8 |
...
## v1.4|條件側三項(2026-08-06)

`docs/25` B 段記著「**條件側三項全未起**」——
Ragic `doc/6` 逐字對應到:「指定日期欄位時間或區間」「指定當前時間」「指定使用者或群組」。

### 一句話裁定:三項是**同一件事**

三者都不是新的條件型別,而是「條件要比的那個值從哪裡來」:
...
故收斂成**虛擬欄位 + `EvalContext`**,條件的形狀(field / op / value)一行不動。
另開一種條件型別的話,求值器、schema、設計器**三處各多一個分支**。

⚠️ 前綴 `$` 撞不到使用者欄位:動態 identifier 鎖 `^[a-z_][a-z0-9_]{0,62}$`。

### 逐字依據(本機鏡像,查證 2026-08-06)

> 「當條件欄位選擇日期、時間欄位時……也可以設定為**是否處於指定日期、時間區間**。」
> 「你也可以針對**當前時間**設定指定日期、時間或區間。還能將**每日指定時間**設為指定條件。」
> 「當條件欄位選擇使用者欄位時,可以設定特定使用者或是群組為指定條件。
> 另外也可以針對**登入使用者**設定特定使用者或是群組為指定條件。
> 當條件包含多個群組時,可以設定為:**需屬於／不屬於其中任一群組,或必須屬於／不屬於所有指定群組**。」

四種群組語意都做了 —— 「不屬於任一」與「不屬於全部」在實務上是兩件不同的事。

### 三個實作上的決定

1. **運算子清單與 `FILTER_OPERATORS` 分家**。後者會編成 **SQL WHERE**;
   把 `between` / 群組運算子加進去,它們會漏進查詢路徑,而那裡沒有實作
   —— 結果是**無聲的無效條件**(篩選看起來套了卻沒作用)。
2. **`dailyBetween` 不是大小比較**:22:00–02:00 跨午夜也要成立。
3. **`ctx` 是尾綴選填**:既有呼叫端一行不動;沒給時 `$actor` 為 null
   → 使用者條件**不成立**(而不是誤判為成立)。

### 🔴 伺服器才是說了算的那一邊

條件式必填是伺服器強制(C-3)。故後端求值時 `$actor` **一律取自後端解析的 actor**,
不吃 client 送來的任何東西 —— 否則「只有主管才必填」的規則,
打 API 的人自己說他是主管就繞過去了。
`/api/authz/me` 新增的 `actorId` / `groupIds` **只給畫面用**,不是授權來源。

整合測試釘的正是這一點:同一條規則,`x-dev-actor: 7` 被擋、`8` 存得進去。
若 `$actor` 沒真的接到後端的 actor,兩次會一模一樣 —— **那就是空過**。

### ⚠️ 條件式隱藏不是安全機制(照抄官方的警語)

Ragic 逐字:「條件式格式的隱藏欄位**只會作用於排版介面上**,於修改資料紀錄或
通知信中仍會顯示該欄位的資料,因此若希望針對不同使用者權限隱藏該欄位時,
建議使用**欄位層級權限設定**。」

我方同此:要真的擋住,用欄位級權限(E 段已出貨),不要用條件式格式。
`$actor` 條件讓「只有主管看得到這一段」在**版面上**成立,**不構成授權**。

### 殘留

| 殘留 | 說明 |
|---|---|
| 「顯示或隱藏欄位值」「顯示或隱藏敘述欄位」 | 效果側仍缺的兩項(10 取 8) |
| 記錄上的**使用者欄位**作為條件 | 目前 `$actor` 是登入者;Ragic 另可拿記錄上的 member 欄比群組 |
| 條件間只能全 AND 或全 OR | 與 Ragic 同一限制(它自己也逐字寫著) |
