...
## 11-bis. 🔴 主檔明細此前不發任何事件(2026-08-04)

`emitInTx` 掛在 `createRecord` / `updateRecord` 那一層,而 `saveWithLines`
自己呼叫 `insertOne` / `updateOne` —— 於是繞過去了:**用主檔明細畫面存的記錄,
webhook 訂閱者收不到任何通知**,事件驅動的整合對這條路徑是瞎的。

⚠️ 與 2026-08-03 修過的「主檔明細從未寫過搜尋索引」是**同一個形狀**,
而且就在**同一段程式碼裡** —— 索引補了、事件沒補。
**一起補的時候漏掉一半,比兩個都沒做更難發現**:兩個都沒有時症狀一致,
只補一半時症狀變成「搜得到但沒通知」,而沒有人會去比對這兩件事。

已補:主檔(建立 / 更新)+ 每一列明細(建立 / 更新)+ 被移除的明細(刪除),
與索引寫入同一段、同一個交易。整合測試 `events-webhooks` 有專條。

**形狀教訓**|橫切關注點(索引 / 事件 / 稽核 / 通知)掛在單筆路徑上時,
**任何繞過那一層的批次或複合路徑都會靜靜地沒有**。批次匯入踩過一次、
主檔明細踩過兩次(索引一次、事件一次)。
...
| W2 | 🔴 **載荷洩漏訂閱者無權的欄位** | **僅送 thin**(只有參照,無欄位值)→ 洩漏面為零。**2026-08-03 複審:維持 thin-only,fat 模式不主動實作** —— 它不是缺口而是一個**還沒有消費者的選配能力**,而實作它等於親手建一個 P0 洩漏面(要在投遞當下解析訂閱主體的欄位 ACL、逐欄白名單、過帳類強制 thin),§0.4 亦已載明**業界無任何廠商明載此題**、兩條硬規則是我方自行推導。**觸發條件明確**:等有客戶提出「webhook 要帶欄位值」時,連同他的實際欄位權限形狀一起設計,屆時獨立走 M0 | **P0** | ✅(以 thin-only 達成)|
...
| **W7** | 🔴 **投遞紀錄無限成長,且是保留期破口** —— 每一列都帶一份完整載荷(業務資料快照)與回應內容,而**沒有任何機制會清掉** | ✅ **2026-08-03**。分兩段而非一刀刪:**30 天**清內容(載荷置空 + `response_body` 清掉 + `pruned_at`)、**一年**整列刪除。理由是兩個問題的時間尺度不同 —— 排查「送出去沒、對方回什麼碼」發生在幾天內,而「我們有沒有把這筆資料送給外部端點」是內控要問的,答案不該跟著內容消失(同回收桶「刪檔案、留列給稽核」的取捨)。**pending 的一律不清**(那是待送內容)。🔴 **連帶必須擋重送**:內容沒了還讓人按重送,會送出一份空載荷**而且回 200** —— 對消費端就是一筆內容突然變空的事件,比明白地說「重送不了」糟得多 → `pruned_at` 同時是重送閘門(410 `DELIVERY_CONTENT_PRUNED`)| P1 | ✅ |
...
| 日期 | 版本 | 變更 | 作者 |
|---|---|---|---|
| 2026-08-03 | v1.1 | **W7 投遞紀錄保留期落地,模組殘留清空**。`webhook_delivery` 每一列都帶完整載荷(業務資料快照)與回應內容,而在此之前**沒有任何機制會清掉** —— 既是無上限成長,也是「業務資料副本無限期躺在另一張表裡」的保留期破口。採兩段:30 天清內容(載荷置空 / `response_body` 清掉 / 記 `pruned_at`)、一年整列刪;pending 一律不清(那是待送內容)。🔴 **設計時就看出的連帶失效**:內容清掉之後若還能按重送,會送出一份空載荷**而且回 200** —— 對消費端是一筆「內容突然變空」的事件,比明白地擋下來糟得多,故 `pruned_at` 同時是重送閘門(410)。migration 0047 + `webhook.retention` 具名 cron(未具名者以 UUID 進 registry,重複註冊偵測不到)。**W2 fat 載荷複審後決定不做**:它不是缺口,是**還沒有消費者的選配能力**,而實作它等於親手建一個 P0 洩漏面;§0.4 已載明業界無廠商明載此題、兩條硬規則屬我方推導 —— 等真有客戶要求時連同他的欄位權限形狀一起設計,屆時獨立走 M0。api 26 綠(events-webhooks)| Claude Code |
