...
> ✅ **狀態:SHIPPED v1.0(2026-07-31;M1–M12 全數完成 + FMEA 收尾)**
...
> **M11 + M12 已完成(2026-07-31)—— 模組收尾**
>
> **M11**|docs/14 三項口徑更正經查已於 M1 完成;**真正的缺口在記憶檔** —— `feedback_frontend_premium_bar` 是跨對話生效的,卻通篇是主張、無證據強度標注。已補六條更正(方向無外部依據 / 不得以效率為由 / 工藝可量測 / 窗口在首次接觸 / 應刪六條無據主張 / 反而有實證的兩條)。
> **FMEA 補完**|14 條中**實際發生 4 條**(U1 部分 / U4 / U7 / U11 / U12),另有**設計階段未預見的 3 條**(U15 批次 rename 撞名 · U16 token 靜默失效 · U17 Hooks 規則違反)。未發生者**不刪**,作為日後修改的守衛清單。
> **docs/25 回填**|升 v2.2 但**覆蓋率誠實不變**(仍 ~113/248 ≈ 46%)—— UX-1 是橫切品質模組,改善的是已交付項目的品質,**不新增功能覆蓋,不灌水數字**。唯一觸及本表者為網格「鍵盤導航」(HTML 表格面已補,Glide canvas 面仍為套件自帶)。
>
> **M12**|產出 `docs/mockups/visawi-s-protocol.html`(VisAWI-S 4 題可互動量表 + 施測協定 + 判讀基準)。
> 🔴 **兩個限制誠實記錄,不用回溯估分掩蓋**:
> 1. **「改版前」基線已無法取得** —— M1–M10 已 shipped,回不去。**M1–M10 的前後對照不存在**,現況只能作為**日後工藝工作的基線**。設計文件原訂「改版前量基線」,實際排序讓它落空。
> 2. **量表需真人施測,不可自評** —— 它量的是受試者主觀評分,由實作者自填等於沒有效度。**我不代填任何分數。**
>
> **首次接觸鏈現況**|登入 / 首頁 / 首次建表三頁之字階已於 M10 全數合規(僅用 12/13/14/24 四階),對比亦由 M9 涵蓋。**Craftsmanship 的施力方向(一致性與概念完整)與已完成的工作重合** —— 字階收斂 / 對比一致 / 間距節奏 / 動效 token 化即「skillful and coherent integration」的具體落實,非額外的工。
>
> **M10 已完成(2026-07-31)**|**字階 17 種 551 處 → 6 階**(12/13/14/16/20/24,地板 12px)。416 處改動、79 檔。
>
> 🔴 **FMEA U11(P0)實際發生,並依 Fiori 原則解掉**|收斂後記錄清單列高 **55→57px、可見列數 14→13** —— **字階收斂真的降低了密度**。依 SAP Fiori 官方「compact 模式**字級不變,只縮元件尺寸與間距**」(即**密度靠間距不靠縮字**)以間距回收:`py-2→py-1.5`、`mt-1→mt-0.5` → **51px / 15 列**,不但未變差反而**多一列**。
> **量測方法**|改動前後各跑一次探針,並以 `git stash` 取得改動前基線 —— **沒有基線就無從判斷 U11 是否違反**。已將探針轉為常設守衛 `e2e/density.spec.ts`(上限取改動**前**的 55px / 14 列),釘住日後不得「順手」放寬。
>
> **CI ②:字級白名單**|`src/components/type-scale.test.ts` 掃全庫,`text-[Npx]` 不在階內即 fail。另釘住**禁「又縮小又調淡」**(最小兩階不得配 `ink-disabled`)—— 此測試當場抓到 **M9 自動分類的誤判**:「唯讀」標記與矩陣註記被歸為停用態,實為**資訊性**文字,已改回 `ink-3`。剩餘 3 處 `ink-disabled` 皆帶 `disabled:` 與 hover 態,確為真停用色。
>
> **驗證**|web **126 單元**(+2 檢查)· lint 0 · build 過 · 全 e2e **71 綠 / 4 失敗**(等同既有基線;為本模組最高通過數)。
>
...
## 7. FMEA(M11 收尾;含**實際發生**紀錄)

> 設計階段列 14 條。**實際發生 6 條**(U4 / U5 / U7 / U11 以及兩條設計階段未預見者),
> 其餘由實作方式避開。**未發生者不刪** —— 它們是日後修改時的守衛清單。

| # | 失效 | 嚴重度 | 實際情形 | 緩解 / 守衛 |
|---|---|---|---|---|
| U1 | rail 收斂後常用設定變兩次點擊 | **P0** | ⚠️ **部分發生**:設定六項滑鼠路徑 1→2 次 | 同批把六項全數加進 ⌘K(收斂前只涵蓋 1/6)→ 鍵盤路徑由「5 項搜不到」**改善為全可達**。⚠️ 頻率假設待上線數據複核 |
| U2 | folder 化 rename 漏改 import | P1 | ✅ 未發生(但撞名發生,見 U12-bis) | 純 rename + build + 全 e2e + grep 舊路徑零殘留 |
| U3 | APG 鍵盤與 Glide canvas 衝突 | P1 | ✅ 未發生 | `collection-view` 明確不動;e2e 斷言其無 grid 標記 |
| U4 | roving tabindex 錯誤致鍵盤陷阱 | **P0** | 🔴 **發生(反向驗證觸發)**|另發現**對偶缺陷**:選取項不在清單時無任何 tabIndex 0,鍵盤**進不來** | `Tab` 一律不攔截;`rovingId` 保證恰有一個停點;逐鍵 e2e |
| U5 | 「最近使用」洩漏跨租戶或越權表單 | **P0** | ✅ 未發生 | 安全性由建構保證(對照授權清單解析);⚠️ 反向驗證顯示**單靠第二層即成立**,key 隔離為縱深防禦 |
| U6 | docs/14 改錯致後續施工偏離 | P1 | ✅ 未發生 | 更正處註明 supersede 來源與日期 |
| U7 | `keepPreviousData` 顯示上一情境資料 | **P0** | 🔴 **發生,且為既有缺陷**:query key 不含租戶,切公司即顯示前一家快取 | 切 org 時 `queryClient.clear()`(一處,勝過 20 個 hook 加前綴) |
| U8 | 進度條自身造成位移 | P1 | ✅ 未發生 | `absolute` 定位不佔流 |
| U9 | 延遲 400ms 使快速操作像沒反應 | P1 | ✅ 未發生 | <400ms 本就瞬時(NN/g);按鈕 disabled 態即回饋 |
| U10 | reduced-motion 誤關功能性動效 | P1 | ✅ 未發生 | 守衛僅作用於裝飾類 |
| U11 | 字階收斂降低單螢幕可見列數 | **P0** | 🔴 **發生**:列高 55→57px、可見 14→13 列 | 依 Fiori「密度靠間距不靠縮字」以 padding 回收 → 51px / **15 列**。轉為常設守衛 `e2e/density.spec.ts` |
| U12 | `ink-4` 拆分把資訊性文字誤歸 disabled | P1 | 🔴 **發生**:「唯讀」標記與矩陣註記被自動分類誤判 | 由 M10 的「禁又縮小又調淡」測試抓到並改回 `ink-3` |
| U13 | 輸入框邊框加深致全站視覺變重 | P1 | ✅ 未發生 | 新增 `--color-line-input` **僅供輸入框**;表格框線以測試釘住不得加深 |
| U14 | 字階大改造成非預期版面破圖 | P1 | ✅ 未發生 | 416 處改動後 build + 全 e2e 零新增失敗 |

**設計階段未預見、實作中發生的三條**

| # | 失效 | 成因 | 守衛 |
|---|---|---|---|
| **U15** | **批次 rename 誤改他目錄同名檔** | 腳本只比對**檔名**未比對**目錄**;`forms/` 亦有 `form-workspace.tsx` | rename 前 `comm -12` 找同名檔;**若兩側都有同名匯出則 type-check 會過而執行期才壞** → 必跑 build + e2e。已寫入 `pitfall_package_path_collision` |
| **U16** | **design token 靜默失效** | Tailwind 4 由 `--transition-duration-*` 產生 `duration-*`;用 `--duration-*` **class 完全不產出**,過場變瞬間完成,而 type-check / lint / build **全數通過** | **驗證方式升級**:`grep` 產出的 CSS 確認 utility 與值,不只看原始碼 |
| **U17** | **Hooks 規則違反(M5 遺留)** | `useGridKeyboard` 置於 early return 之後 → 載入狀態切換時呼叫順序改變 | type-check 抓不到,**只有 lint 會**;M5 當時未跑 `pnpm lint`(流程漏)。已納入每 milestone 必跑 |
