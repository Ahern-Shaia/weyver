...
🔴 **2026-08-04 更正**|上一版這裡寫著「已列為 SOP 的必填項」,
而**當時並不存在任何 SOP 文件** —— 那是一句指向不存在之物的宣稱。
現已(a)把這條警語放進**設計器裡選 `fieldRef` 的當下**(唯一擋得住繞過的地方是那裡,
不是文件),(b)補上 §3-ter 遷移 SOP。

---

## 3-ter. 🔴 前端落地缺口(2026-08-04)—— 後端出貨了,入口沒開

> **這一節記錄的不是新設計,是一個「已裁定、後端已出貨、但使用者碰不到」的缺口。**
> §3-bis 的狀態欄逐字寫著「✅ 已裁定 C 並落地(2026-08-03)」——
> 那句話對**後端**成立,對整條路徑不成立。

| # | 缺口 | 後果 |
|---|---|---|
| 1 | 設計器的簽核人規則清單**沒有 `fieldRef`** | 只能打 API 建。第一約束逐字:「有 API 可以做」不算解決。而 R1 的整個目的就是遷移,這正是遷移的落點 |
| 2 | 前端 `APPROVER_RULES` 鏡射**漏了 `fieldRef`** | 比選不到更糟:用 API 建過的租戶,`z.enum` 解不到 → 整個 def parse 失敗 → **簽核設定區塊整塊消失**。同一個檔案上面的註解正好警告過這個形狀 |
| 3 | 三個 manager 規則**可選但恆解不出人** | 它們靠角色樹的父子關係解析,而角色頁 UI **刻意是平的**(`authz` 0-bis 項 1:客戶是行政兼職,階層理解成本才是瓶頸)。從畫面把角色建起來的租戶永遠沒有父角色 → 選了之後要等到**送簽當下**才炸 |

**處置**|
- 前端鏡射補齊 `fieldRef` + `approverField`。
- 設計器加「記錄上的欄位」+ member 欄選擇器(候選就是本表的 member 欄;沒有時該項停用並說明)。
- 三個 manager 規則在**沒有任何角色設過父角色**時停用,選項文字直接寫
  「(尚未建立角色階層)」—— 不讓人選一個註定失敗的規則。
- 繞過風險的警語放進**選 `fieldRef` 的當下**。

⚠️ **沒有做的事(誠實記錄)**|**沒有**為了讓 manager 規則可用而去開角色階層 UI ——
那條「刻意平掉」的裁定有它自己的理由(`role-tree.tsx` 檔頭逐字),
不該為了本模組單方面推翻。想要「送給我的主管」的租戶,現在的答案是 `fieldRef`。

---

## 3-ter-bis. 遷移 SOP|Ragic 直屬主管欄 → Weyver

> 承 §3-bis:轉換退化成「指定是哪一欄」,但有三個**必做**步驟,漏一個就會出事。

| # | 步驟 | 為什麼是必做 |
|---|---|---|
| 1 | 把 Ragic 的「直屬主管」欄匯入為本表的 **member 欄**(每筆記錄各自持有) | Ragic 的主管本來就住在欄位裡;翻譯成組織結構會生出一堆機器命名的角色,且客戶事後調不動 |
| 2 | 簽核流程該關選「**記錄上的欄位**」,指向步驟 1 那一欄 | 這是 `fieldRef` 的設定入口(2026-08-04 起在設計器裡) |
| 3 | 🔴 到權限頁把該欄位**對申請人設為唯讀** | **唯一擋得住繞過的步驟**。程式擋得住「指到本人」,擋不住「指到自己的下屬」—— 申請人改得動那一欄就等於自己指定誰來核准他 |

**驗收**|以一位申請人的身分:(a) 該欄在填單畫面是唯讀的;(b) 送簽後簽核人正是欄位所指的人;
(c) 把該欄清空後送簽會**當場被擋**並指名是哪一關(不是走到一半才炸)。
