...
| # | 失效 | 嚴重度 | 緩解 |
|---|---|---|---|
| **T7** | 🔴 **扇出無上限**。`depth` 限的是**鏈長**不是**分支**:一張表掛 T 條 pushTo 觸發器,一次存檔 → T 筆 → 每筆再 T 筆…… 深度 5 的最壞情況是 **T⁵**(T=20 → 320 萬筆)| **P0** | ✅ **已緩解**:`event_outbox.root_event_id` 標出源頭,**單一使用者動作的後代總數上限 100**。實測拿掉這道閘後,深度閘擋不住(記錄 id 衝到 273、耗時 11.5 秒) |
...
### ✅ T7 已落地(2026-08-06)—— 兩道閘缺一不可

`depth` 限鏈長、`root_event_id` 計數限總量。**單獨任一道都擋不住乘法。**

**為什麼限「後代總數」而不是「每個事件最多跑幾條觸發器」**|
一張表掛 10 條各做各的事的觸發器完全正常,限分支會誤傷合法設定。
真正該問的是「**一次存檔最多可以連帶產生幾筆資料**」——
那是使用者理解得了、資料庫也真的承受的那個量。

`root_event_id` 為 `NULL` 代表「這個事件自己就是源頭」(使用者直接造成);
worker 建出子記錄後把子事件掛到 `parent.root_event_id ?? parent.id`。
部分索引只蓋非 NULL 的那些 —— 絕大多數事件是使用者直接造成的,不必進索引。

⚠️ **上限 100 無外部依據**(同 `MAX_ATTEMPTS`)。查過的兩家都幫不上:
Airtable 的煞車是**月配額**(Free 100 / Team 25,000 / Business 100,000 runs
per workspace per month),那是**計費單位不是連鎖單位**;Teable 沒有。
選 100 的理由:`pushTo` 一條觸發器只建**一筆**(不隨資料量放大),
合法的連鎖本來就小;一次存檔連帶產生超過 100 筆幾乎必然是設定錯誤。

**停下來時要分得出理由**|`detail.reason` 為 `depth` 或 `cascade`。
只寫「停了」的話,設計者不知道該縮短鏈還是減少觸發器數量。

**測試刻意做成乘法的形狀**(A/B 各掛 3 條互推,每代 ×3),不是一條鏈 ——
只驗鏈的話深度閘就會把它擋掉,**測不出扇出有沒有被管**。
