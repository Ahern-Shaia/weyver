...
## 4.6 M2 落地(2026-07-29)

| 項目 | 狀態 |
|---|---|
| **未知選項 `create` 模式** | ✅ schema 一直接受 `create`,但**實作只有 error 路徑** —— 使用者選了「自動新增選項」會在寫入時整批失敗。現在:plan 階段就列出是哪些值(error 模式擋、create 模式警告),commit 前先把缺的選項加進欄位(只加不改不刪)|
| **合併儲存格** | ✅ 值只存在左上角,其餘是空的 → 直接匯入靜默產生空白。舊 Excel 的單頭欄(訂單編號跨多列明細)幾乎必然合併。以左上角值填滿整個範圍並回報數量,UI 明說「已用左上角的值填滿」|
...
順帶補兩個下拉的 `aria-label`(匯入方式 / 比對欄位),原本無標籤。
