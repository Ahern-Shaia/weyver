...
| **變更歷史 / 通知洩漏** | **Ragic 官方明載**:Hidden 欄「只隱藏版面介面,資料仍會出現在**變更歷史與記錄更新通知**」 | ✅ **通知已安全**(H-1 `safeTitle()` 型別上不接受欄位值);✅ **變更歷史待查 → 查完:Weyver 目前沒有變更歷史**(2026-08-04,見下) |
...
### ✅ 公式旁路:查證結果與修法(2026-08-04)

**證實會洩漏。** 讀取路徑是「**先算公式,再遮罩隱藏欄**」——
遮罩刪的是隱藏欄自己的值,而公式欄是**另一個欄**,不會被刪,
但它的值是用隱藏欄算出來的。

> `月薪` 對某角色隱藏、`年薪 = {月薪} * 12` 沒隱藏
> → 該角色看得到年薪,**除以 12 就還原出月薪**。遮了等於沒遮。

**修法**|`FormulaService.fieldsTaintedByHidden()` 算**傳遞閉包**,
`maskRead` 連同被污染的公式欄一起刪。
⚠️ **必須是閉包不能只看一層**:`A = {B} * 2`、`B = {隱藏欄} + 1` ——
A 沒有直接引用隱藏欄,一樣洩漏。整合測有一條專釘這個。

⚠️ **刻意不採「把隱藏欄當 0 去算」** —— 那會給出一個**看起來正常但錯的數字**,
比看不到更糟(同 `pivot-and-charts` 的裁定:寧可具名不可用,不要靜默給錯的值)。

**四個讀取路徑全部掃過**(列表 / cursor 分頁 / 單筆 GET ×2)——
只修列表等於沒修,單筆 GET 照樣洩。

🔴 **順帶揪出一個無關但更嚴重的靜默 bug**|`addField` **不註冊公式** ——
原本只有 `createForm` 呼叫 `defineFormulaFields`,
於是「**建完表之後才加的公式欄永遠算不出來**」:欄位出現在畫面上、值恆為 `null`、沒有任何錯誤。
而「建完表再加欄」正是 2D 設計器的日常動作。已修,並因此讓語法錯誤也會在加欄時就擋下(fail closed)。

---

### ✅ 另外兩條旁路的查證結果(2026-08-04)

#### yes/no oracle —— **目前找不到可達的實例**

oracle 的形狀是「用是/否的回應差異推斷看不到的值」。逐一走了三條最可能的路:

| 可能的探測面 | 查證 | 結論 |
|---|---|---|
| 唯一欄探測(對隱藏的唯一欄送值,由「重複」訊息反推該值存在) | `assertWritable` 以**寫白名單**擋下 —— 看不到的欄也寫不進去,連探測都送不出去 | ✅ 結構上不可達 |
| 篩選 / 排序 / 快速搜尋反推 | 早前已修(commit `41155c4`),隱藏欄不得進 WHERE / ORDER BY / 搜尋 | ✅ 已擋 |
| 公式反推 | 本輪修(見上) | ✅ 已擋 |

⚠️ **這不等於「沒有 oracle」** —— 只表示**我找過的三條路都不通**。
下表「推斷但必防」列的聚合洩值 / tracker attack / GROUP BY 基數 / 分頁 total count
**依然未查**,而那幾條正是 oracle 最典型的棲地。

#### 變更歷史 —— **Weyver 目前沒有這個功能**

Ragic 官方明載 Hidden 欄「資料仍會出現在**變更歷史**與記錄更新通知」。
查證:全庫**沒有 `record_history` 之類的表,也沒有任何記錄變更歷史端點**
(`db/schema.ts` 只有 `ddl_audit` / `action_audit` / `approval_step_log` / `auth_audit`,
皆非欄位值層級的變更軌跡;`action_audit.detail` 僅由本人以自己的冪等 key 取回)。

👉 **所以這不是待修的 bug,是待記的設計約束**:
**做變更歷史的時候,欄位級權限必須一起做進去** ——
Ragic 的這個洩漏是因為歷史功能先做、權限後補。我們還沒做,還來得及不要重蹈。
已寫進本節,做該功能時應回頭讀。
...
| **1 角色樹 UI 收斂** | 預設 UI 收成平面角色 + 群組,樹降進階 | ⚠️ **建議其實已是現況,但 UI 在說謊**。`createRole` **永遠傳 `parentId: null`**,前端**沒有任何入口**能建立或調整階層 —— 也就是說 UI 早就是平的。後端反而**有完整的樹**(`createRole` 收 `parentId`、`PATCH .../parent` 可改掛載點、有深度上限),schema 成本已付。問題出在**畫面暗示了它做不到的事**:資料夾圖示 + 無說明的 `depth` 縮排 + 「部門」字樣 | ✅ **已處理**:改用「一群人」圖示(資料夾暗示「裡面裝得下別的角色」)、只在**真的存在** `depth > 0` 的角色時才解釋縮排並明講「此處不調整階層」。**不開放前端調階層** —— 對齊本項建議與競品(Airtable / Baserow / NocoDB / Ragic 皆無角色樹,用資源層級繼承,我方對應物為分類授權 → 表單繼承) |
| — 其餘查詢旁路查核 | | | **未做**,列 P1 |

**四項複核完畢**|項 4 維持 · 項 7 已落地 · 項 1 已處理(結論為「不做階層 UI」而非「收斂階層 UI」)· 旁路查核 P1。
