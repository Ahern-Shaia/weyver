...
> **狀態**|✅ **v2.0(2026-08-01)· 已 cascade 進權威文件 `docs/14 §0.4`**(本文為證據與裁定紀錄,規則本身以 docs/14 為準)
...
## 5-bis. 🔴 視覺實走(v2.0 補課 —— 原分析只讀原始碼,沒看過畫面)

> **為什麼有這一節**|v1.x 全文取自原始碼(`theme.ts` / `colors.module.css`)。
> 那能讀出**數值**(圓角 8px、陰影 alpha 0.05、字階 11/12/14/17/21),
> 讀不出**畫面**(版面怎麼組、密度多鬆、哪些元素被著色、哪裡有信任訊號)。
> 結果是所有裁定都落在「數字差異不構成改的理由」,**視覺上淨改變為零** ——
> 這正是 review 時被指出的問題。
>
> **補法**|`docker run metabase/metabase` 自架一套,以 Playwright 實走並截圖 27 張
> (1600×1000 @2x,存於 `docs/research/ui-screenshots/metabase/`,已登錄 MANIFEST v4)。
> 行銷網站的圖是裁切過的,不能用來判斷密度。
> ⚠️ **法律界線**|截圖僅供研究,**不得像素級複製**(CLAUDE.md 法律紅線;此庫已有 412 張競品截圖同性質)。

### 5-bis.1 讀原始碼**讀不到**的五項發現

| # | 實走所見 | 對照 Weyver | 裁定 |
|---|---|---|---|
| **V1** | 表格**沒有直向格線**,水平分隔極淡;**欄頭文字是品牌藍**不是灰;**外鍵值渲染成連結色** | 我方全框線 + 灰欄頭;關聯欄位顯示為**純文字** | 全框線 ❌ 不動(視覺簽名);🔴 **關聯值要看得出可點** —— 見 5-bis.2 |
| **V2** | 步驟式建構器的每個動作**各有柔色底 + 同色文字**(Filter 紫 / Summarize 綠 / Join 藍),步驟本身是撐滿寬度的淡色橫幅 | A6「動作專屬色」原本只是抽象概念 | ⚠️ **形狀採用、粉彩不採用**(R2) |
| **V3** | **管理區整條頂欄換成紫色** —— 一眼知道「在改設定,不是在用資料」 | 設定中心與工作區同一套 chrome,只有側欄選中項不同 | 🔴 **值得採用(改良版)**,另立 OQ |
| **V4** | 篩選面板的欄位清單依**本表 / 關聯表**分組,關聯表以 `+` 前綴並可展開 | 我方篩選是**平鋪**,有 lookup 時會混在一起 | ✅ 採用其資訊結構 |
| **V5** | 底部狀態列同時給**筆數上限**與**查詢耗時**(`Showing first 2,000 rows` · `152ms`) | docs/14 已列狀態列為信任訊號,但**沒有耗時** | ✅ **採用**(低成本高信任) |

### 5-bis.2 V1 拆開講 —— 別把「不搬全框線」當成「什麼都不用學」

Metabase 的表格語言(無直向格線 + 藍欄頭 + 連結色外鍵)整體**不適合我方**:
我方是單據語言,全框線對應紙本、對應會計逐格核對,是 docs/14 明列的視覺簽名。

**但其中一項是功能性缺口,不是風格差異**:Metabase 讓**外鍵值長得像連結**,
使用者因此知道那格可以點進去。Weyver 有 Link&Load 關聯欄位,
目前在列表與記錄頁都是**純文字** —— 可點,但看不出可點。
這不是「要不要更好看」,是**可用性訊號缺席**;`--color-link` token 早就存在。

### 5-bis.3 V3 拆開講 —— chrome 換色是模式編碼,不是裝飾

docs/14 §0.1 禁裝飾配色;§0.2(2026-07-28)已經把界線講清楚:
**判準不是「有沒有顏色」,是「顏色有沒有在傳遞資訊」**。
管理模式與資料模式的誤操作代價差一個量級(改欄位型別 vs 填一筆資料),
用 chrome 承載這個區別與「選項顏色是類別編碼」同源。

⚠️ **不照抄的部分**|Metabase 用的是**另一個色相**(紫)。我方應用**自身主色的深色階**
(`--color-primary-d`),理由:(a) 三配色主題下不能憑空多一個色相;
(b) 白牌客戶換色時管理區要跟著換,否則會出現「客戶的品牌色 + Metabase 的紫」。

### 5-bis.4 誠實記錄:實走**沒有**推翻的既有裁定

| 項目 | 實走所見 | 仍維持 |
|---|---|---|
| 密度 | 列高明顯較我方鬆,留白大 | ❌ 不放鬆 —— BI 是掃描少量結果,ERP 單據是逐列核對 |
| 圓角 / 柔色 | 建構器按鈕圓角大、粉彩底 | ❌ 維持 6px 上限與禁裝飾配色 |
| IA | 首頁是儀表板 / 集合 | ❌ 與 docs/24「主畫面非 KPI 儀表板」直接衝突 |

**結論**|實走**沒有**讓「該不該改風格」的答案翻盤 —— 我方的嚴謹取向依然成立。
它翻盤的是**「有沒有東西可學」**:光讀原始碼,V1b / V3 / V4 / V5 這四項
**不可能出現在分析裡**,因為它們都不是數值。
...
| 日期 | 版本 | 變更 | 作者 |
|---|---|---|---|
| 2026-08-01 | v2.0 | 🔴 **補上視覺實走(§5-bis)**。review 指出「看不出 docs/28 對 Weyver 在視覺上有提高設計」—— **屬實**:v1.x 全部取自原始碼,讀得到數值讀不到畫面,於是每一條視覺裁定都落在「數字差異不構成改的理由」,淨視覺改變為零。補法為自架 Metabase 並實走截圖 27 張入庫。新發現四項讀原始碼**不可能發現**的(關聯值無可點訊號 / 管理區 chrome 換色作模式編碼 / 篩選欄位依關聯分組 / 狀態列給查詢耗時);同時誠實記錄實走**沒有**推翻密度、圓角、IA 三項既有裁定 | Claude Code |
