...
- ❌ **原句錯誤,且是全庫最嚴重的一條**|Ragic 有設計手冊**第 8 章 Ragic AI 小幫手**:AI 建資料庫(需求→自動生成一系列表單 + 關係圖)、NL 查詢、**AI Agent(6 觸發 × **10** 動作〔2026-08-06 開檔覆查更正:原記 11,把 HTML 表頭列算進去了〕,含 `CREATE_RECORD`/`MODIFY_RECORD`)**、MCP(唯讀繼承權限)、AI 公式助手、單據抽取(拍發票自動填單)、私有主機本地模型 + BYO key
